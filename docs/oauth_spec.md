...
- Inventory the session consumers outside the monolith and document their interfaces:
  ...
  - `status2` is a Go WebSocket service with the same deployment model, subscribing to Redis status channels for browsers.【F:wepppy/weppcloud/routes/usersum/dev-notes/status2.spec.md†L1-L85】
...
- Add token verification middleware to each service:
  ...
  - Preflight2 and Status2: extend the Go servers to parse and verify JWTs (using a JWKS cache) before accepting WebSocket upgrades; surface user context in structured logs for auditing.【F:services/preflight2/internal/server/server.go†L75-L118】【F:wepppy/weppcloud/routes/usersum/dev-notes/status2.spec.md†L58-L112】
