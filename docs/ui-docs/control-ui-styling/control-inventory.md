...
| BAER Upload | Pure macros (`disturbed_sbs_pure.htm`) · legacy `_base.htm` | `baer.js` | `disturbed_bp` (`tasks/upload_sbs` via `/upload`, `tasks/remove_sbs`, `tasks/set_firedate`) | SBS raster upload, radio-mode switches, uniform builders | Uses `FormData`, refreshes map overlays, conditional content by `ron.mods`; JS now delegates via `data-sbs-*` hooks so both layouts stay functional |
| Road Upload | `_base.htm` | Inline wiring via `run_page_bootstrap.js` (no dedicated module) | _TBD_ – legacy `disturbed` tasks (`/tasks/upload_road/`, `/tasks/remove_road/`) require confirmation; use `/upload` prefix if revived | `.geojson` upload, upload/remove buttons | Needs explicit JS controller + active route check before standardization |
...
| Climate | Pure macros (`controls/climate_pure.htm`) | `climate.js` | `climate_bp` (`tasks/update_climate`, `tasks/upload_cli` via `/upload`, `query/*`) | Radio groups, select inputs, `.cli` upload | Uses catalog-driven sections, delegated events, and StatusStream panels |
...
| Treatments (optional) | Pure macros (`controls/treatments_pure.htm`) | `treatments.js` | `treatments_bp` (`tasks/set_treatments_mode`) & `/rq/api/build_treatments` (use `/upload` when attaching rasters) | Mode switch (per hillslope vs upload), raster upload, builder button | Pure control with StatusStream + upload helper. Renders as standalone section when treatments mod enabled |
...
| `input_upload_landuse` | file (.img/.tif) | Upload thematic landuse raster | User chooses local file; expects SBS-aligned values | Consumed by `POST /upload/runs/<runid>/<config>/rq/api/build_landuse` → `Landuse.parse_inputs` | Only visible when `landuse_mode == 4`; ensure macro annotates accept list & describes required schema |
...
| `btn_build_landuse` | button | Submit landuse build job | N/A | Triggers `POST /runs/<runid>/<config>/rq/api/build_landuse` (use `/upload` prefix when a file is attached) | ControlBase disables/enables button; ensure macros tag `data-command` for HTMX replacement |
...
| `input_upload_cli` | file (.cli) | Upload custom climate file | Local `.cli` file, validated by `Climate.set_user_defined_cli` | `POST /upload/runs/<runid>/<config>/tasks/upload_cli/` | Only enabled when build mode = user-defined; macro needs to surface allowed extension |
...
| `input_upload_landuse` | file (.img/.tif) | Upload treatment map | User-provided raster; validated & reprojected during build | `POST /upload/runs/<runid>/<config>/rq/api/build_treatments` (reuses `build_landuse_rq` pipeline) | Pure control uses `save_run_file` (100&nbsp;MB limit, `.tif/.img` allow-list); field hidden unless mode `4` selected |
...
| `btn_build_treatments` | button | Apply treatments | N/A | Calls `/rq/api/build_treatments` (use `/upload` prefix when a file is attached), which triggers `build_landuse_rq` with treatment context | RQ job shares WS channel `treatments`; button wired via delegated handler |
...
| `input_upload_sbs` | file (.tif/.img) | Upload Soil Burn Severity raster | `POST /upload/runs/<runid>/<config>/tasks/upload_sbs/` → map saved/validated via `Baer.validate` | Accepts uint8/short thematic rasters; macros must note projection expectations |
| Upload/Remove buttons (`upload_sbs`, `remove_sbs`) | button | Manage SBS map | `Baer.upload_sbs()` / `remove_sbs()` call `tasks/upload_sbs` (via `/upload`) and `tasks/remove_sbs` | Buttons hidden in readonly mode; expose `data-sbs-action="upload/remove"` so delegated handlers work in both legacy and Pure layouts |
...
- `disturbed_bp`: `/tasks/upload_sbs/` (via `/upload`), `/tasks/remove_sbs/`, `/tasks/set_firedate/`, `/tasks/modify_burn_class`, etc.
...
| --- | --- | --- | --- |
| Scenario builder (`omni_scenarios_pure.htm`) | Dynamically repeats: `scenario` select (values map to `OmniScenario.parse`); scenario params (`canopy_cover`, `ground_cover`, `ground_cover_increase`, `base_scenario`, `sbs_file`) depending on chosen type | `Omni.serializeScenarios()` builds `FormData` → `POST /upload/rq/api/run_omni` when SBS files are attached; server saves uploads in `omni/_limbo` and calls `Omni.parse_scenarios` | Macro now drives the card layout; metadata still pulled from JS until descriptor work lands |
...
| `input_upload_*` (map uploads) | File inputs for ash maps | Posted to `/upload/rq/api/run_ash` when map files are attached (handled by `_task_upload_ash_map` + `save_run_file`) | Client-side + backend validation enforce <100&nbsp;MB and `.tif/.tiff/.img` extensions; Pure template passes data attributes for future enhancements |
