...
_Date: 2025-10-18 (initial catalogue aligned with the Control UI Final Implementation Blueprint)_
...
| Set Outlet | `_base.htm` | `outlet.js` | `watershed_bp` (`tasks/set_outlet`, `tasks/clear_outlet`) | Map-triggered hidden inputs, select controls | Relies on MapController events and ControlBase for status updates |
...
| Landuse | `_base.htm` | `landuse.js`, `landuse_modify.js` | `landuse_bp` (`tasks/set_landuse_mode`, `tasks/modify_landuse`, `query/*`) | Multi-selects, locale-controlled lists, advanced modify forms | Deep branching on `ron.locales`; numerous AJAX helpers for map selections |
...
| Treatments (optional) | `_base.htm` (`treatments.htm`) | `treatments.js` | `treatments_bp` (`tasks/set_treatments_mode`) & `/rq/api/build_treatments` | Mode switch (per hillslope vs upload), raster upload, builder button | Not mounted on default runs0 but shares ControlBase patterns and uses RQ |
| RHEM | `_base.htm` | `rhem.js` | `rhem_bp` (`tasks/run_rhem`, `tasks/download_rhem`) | Parameter toggles, run buttons | Alternate model path when `ron.mods` enables RHEM |
| Observed Data | `_base.htm` | `observed.js` | `observed_bp` (`tasks/run_model_fit`, `report/observed`) | CSV upload, stat selection | Uses `FormData`, renders comparisons in summary area |
| Debris Flow | `_base.htm` | `debris_flow.js` | `debris_flow_bp` (`tasks/run_debris_flow`) | Numeric inputs, conditional toggles | PowerUser gated; pushes RQ status to shared widgets |
| Ash (WATAR) | `_base.htm` | `ash.js` | `watar_bp` (`tasks/set_ash_*`, `report/ash`) | Mode radios, numeric params, optional map uploads | Inline script hydrates parameter presets; interacts with Disturbed/BAER state |
| DSS Export | `_base.htm` | `dss_export.js` | `export_bp` (`tasks/build_dss_package`) | Export type buttons, option selects | Packages run artefacts; feeds status + download links |
| Omni Scenarios | `_base.htm` (`controls/omni/omni_scenarios.htm`) | `omni.js` | `omni_bp` (`tasks/save_scenario`, `tasks/apply_contrast`) | Dynamic card builder, file uploads, multi-selects | JS builds markup; bypasses static template assumptions |
| Omni Contrast Definition | `_base.htm` (`controls/omni/omni_contrasts_definition.htm`) | `omni.js` | `omni_bp` | Textareas, scenario selectors | Shares metadata with Omni scenarios; needs consistent macro inputs |
| Team / Collaborators | `_base.htm` | `team.js` | `project_bp` (`tasks/adduser`, `tasks/removeuser`, `report/users`) | Email entry, remove buttons | Uses ControlBase for status/logging; summary renders collaborator list |
...
- `ws_client.js`: Redis/Status2 WebSocket bridge; ControlBase attaches per panel.
...
- `run_page_bootstrap.js.j2`: seeds globals (`runid`, `config`, `site_prefix`) and instantiates each controller; also wires cross-control events (e.g., BAER map refresh).
...
| `landuse_db` | select | Select raster catalog for gridded mode | Locale-specific catalog paths (NLCD, CORINE, local overrides) assembled inline from `ron.locales` | `POST /runs/<runid>/<config>/tasks/set_landuse_db/` → `Landuse.nlcd_db` | Displayed only when `landuse_mode == 0`; disabling copy/paste styling critical when converted to macros |
| `landuse_single_selection` | select | Pick management class for single-landuse mode | `landuseoptions` list supplied by NoDb (`Landuse.landuseoptions`) | Submitted with `set_landuse_mode` request → `Landuse.single_selection` | Displayed when `landuse_mode == 1`; JS triggers `handleSingleSelectionChange` to persist immediately |
...
| Field ID | Input Type | Label / Purpose | Options & Data Source | Backend Binding | Visibility / Notes |
| --- | --- | --- | --- | --- | --- |
| `treatments_mode` | radio group | Pick workflow (per-hillslope selection vs uploaded raster) | Values map to `TreatmentsMode` (`1` selection, `4` map upload) | `POST /runs/<runid>/<config>/tasks/set_treatments_mode/` → `Treatments.mode` | Mode `4` exposes file upload + management mapping; mode `1` uses single-selection UI |
| `input_upload_landuse` | file (.img/.tif) | Upload treatment map | User-provided raster; validated & reprojected during build | `POST /runs/<runid>/<config>/rq/api/build_treatments` (reuses `build_landuse_rq` pipeline) | Only required when mode == `UserDefinedMap`; macros must surface file requirements |
| `treatments_single_selection` (implied) | select | Choose treatment when specifying hillslopes | Options drawn from `treatmentoptions` context | Submitted with `set_treatments_mode` to update selection | Appears alongside `textarea` when checkbox-driven selection is enabled |
| `btn_build_treatments` | button | Apply treatments | N/A | Calls `/rq/api/build_treatments`, which triggers `build_landuse_rq` with treatment context | RQ job shares WS channel `treatments`; ensure `_pure_base.htm` registers command button |
...
| Treatments table | `treatments.treatments_lookup` | Displayed in `treatments.htm` map upload section to explain raster coding | Values derived from NoDb lookup; keep table accessible |
...
| --- | --- | --- | --- |
| Scenario builder (`omni_scenarios.htm`) | Dynamically repeats: `scenario` select (values map to `OmniScenario.parse`); scenario params (`canopy_cover`, `ground_cover`, `ground_cover_increase`, `base_scenario`, `sbs_file`) depending on chosen type | `Omni.serializeScenarios()` builds `FormData` → `POST /rq/api/run_omni`; server saves uploads in `omni/_limbo` and calls `Omni.parse_scenarios` | Externalize scenario metadata (labels, required params, predicate checks such as `Disturbed.has_sbs()`) so macros can generate controls without inline JS duplication |
| Omni run trigger | `btn_run_omni` command button | Enqueues `run_omni_scenarios_rq`; ControlBase listens on `omni` channel | Preserve command ID for WebSocket reuse; HTMX scripts must keep multipart submission |
| Contrast definition (`omni_contrasts_definition.htm`) | `omni_contrast_objective_parameter`, `omni_control_scenario`, `omni_contrast_scenario` selects; `contrast_cumulative_obj_param_threshold_fraction` numeric; (additional optional fields consumed by `Omni.parse_inputs`: `omni_contrast_hillslope_limit`, `omni_contrast_hill_min_slope`, `omni_contrast_hill_max_slope`, `omni_contrast_select_burn_severities`, `omni_contrast_select_topaz_ids`) | Posted to `/rq/api/run_omni_contrasts`; `Omni.parse_inputs` maps values to enum fields/properties | Template currently exposes core parameters only; macro refactor should surface advanced inputs (perhaps collapsible) and reuse shared enum option lists |
| Scenario reporting | Summary slot populated via `Omni.report_scenarios()` fetching `report/omni_scenarios/` | Triggered after `OMNI_SCENARIO_RUN_TASK_COMPLETED` event | `_pure_base.htm` summary region should host the rendered HTML; macros need to reserve placeholder ID |

### Observed Data Control
| Field ID | Input Type | Label / Purpose | Backend Binding | Visibility / Notes |
| --- | --- | --- | --- | --- |
| `observed_text` | textarea | Paste CSV-formatted observed series (Date + measures) | `POST /runs/<runid>/<config>/tasks/run_model_fit/` → `Observed.parse_textdata`/`calc_model_fit` | Control shown when climate enables observed datasets; macro should retain instructions list |
...
| Field ID | Input Type | Label / Purpose | Backend Binding | Visibility / Notes |
| --- | --- | --- | --- | --- |
| `ash_depth_mode` radios | Select depth entry mode (depth/load/upload) | `Ash.handleDepthModeChange` updates NoDb via JS; map uploads use `upload_maps` endpoints | Determines which fieldset (depth, load, map upload) renders; macros must manage show/hide + initial state |
| `fire_date`, `ini_black_depth`, `ini_white_depth`, etc. | Numeric/text inputs for ash parameters | Stored on `Ash` NoDb via AJAX posts | Inline script populates defaults per model (ANU/Alex); migration should extract data to metadata blocks |
| `input_upload_*` (map uploads) | File inputs for ash maps | Posted to ash-specific routes (e.g., `tasks/upload_ash_map`) via `Ash` controller | Keep accept list and instructions; ensure file inputs integrate with `_pure_base.htm` |
...
2. Extend the audit to cover:
   - Controls gated behind other run modes or blueprints (e.g., Batch Runner, treatments workflow, additional report modals).
   - JavaScript modules without direct template counterparts (`project.js`, `disturbed.js`) to plan macro data contracts.
3. Annotate field-level metadata requirements (label, locale availability, unit hints) for each control so NoDb descriptors can be authored in parallel with the Pure.css migration.
