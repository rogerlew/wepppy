...
| BAER Upload | Pure macros (`disturbed_sbs_pure.htm`) · legacy `_base.htm` | `baer.js` | `disturbed_bp` (`tasks/upload-sbs` via `/rq-engine/api`, `tasks/remove_sbs`, `tasks/set_firedate`) | SBS raster upload, radio-mode switches, uniform builders | Uses `FormData`, refreshes map overlays, conditional content by `ron.mods`; JS now delegates via `data-sbs-*` hooks so both layouts stay functional |
| Road Upload | `_base.htm` | Inline wiring via `run_page_bootstrap.js` (no dedicated module) | _TBD_ – legacy `disturbed` tasks (`/tasks/upload_road/`, `/tasks/remove_road/`) require confirmation; use `/rq-engine/api` for uploads if revived | `.geojson` upload, upload/remove buttons | Needs explicit JS controller + active route check before standardization |
...
| Climate | Pure macros (`controls/climate_pure.htm`) | `climate.js` | `climate_bp` (`tasks/update_climate`, `tasks/upload-cli` via `/rq-engine/api`, `query/*`) | Radio groups, select inputs, `.cli` upload | Uses catalog-driven sections, delegated events, and StatusStream panels |
| RAP Timeseries | Pure macros (`controls/rap_ts_pure.htm`) | `rap_ts.js` | `rap_ts_bp` (`tasks/run_rap_ts`) & `/rq-engine/api/runs/<runid>/<config>/acquire-rap-ts` | Action button queues RAP TS acquisition job; status stream reports progress | Secondary reporting control; mirrors StatusStream panel contract |
| WEPP | Pure macros (`controls/wepp_pure.htm`) | `wepp.js` | `wepp_bp` (`tasks/run_wepp`, `tasks/download_results`), `/rq-engine/api` job endpoints | Run/queue buttons, advanced options include stack | Core model execution; uses RQ polling and advanced option includes |
| Treatments (optional) | Pure macros (`controls/treatments_pure.htm`) | `treatments.js` | `treatments_bp` (`tasks/set_treatments_mode`) & `/rq-engine/api/runs/<runid>/<config>/build-treatments` | Mode switch (per hillslope vs upload), raster upload, builder button | Pure control with StatusStream + upload helper. Renders as standalone section when treatments mod enabled |
...
| Debris Flow | Pure macros (`controls/debris_flow_pure.htm`) · legacy `_base.htm` | `debris_flow.js` | `debris_flow_bp` (`tasks/run_debris_flow`) & `/rq-engine/api/runs/<runid>/<config>/run-debris-flow` | Single action button triggers debris-flow analysis (PowerUser only) | `controlBase.attach_status_stream` handles telemetry; fallback panels let legacy markup stream without additional shims |
...
| DSS Export | Pure macros (`controls/dss_export_pure.htm`) · legacy `_base.htm` | `dss_export.js` | `/rq-engine/api/runs/<runid>/<config>/post-dss-export-rq` (queues `post_dss_export_rq`) | Mode radios (select channels or by order), optional filters, export button | RQ-backed task with StatusStream logging; legacy template retained for classic runs |
...
- `microservices/rq_engine`: job status (`/rq-engine/api/*`) polled by ControlBase across WEPP, treatments, Omni, etc.
...
| `input_upload_landuse` | file (.img/.tif) | Upload thematic landuse raster | User chooses local file; expects SBS-aligned values | Consumed by `POST /rq-engine/api/runs/<runid>/<config>/build-landuse` → `Landuse.parse_inputs` | Only visible when `landuse_mode == 4`; ensure macro annotates accept list & describes required schema |
...
| `burn_shrubs`, `burn_grass` | checkbox | Opt-in SBS-driven vegetation adjustments | Boolean toggles (`on/off`) | Parsed in `/rq-engine/api/runs/<runid>/<config>/build-landuse` and written to `Disturbed.burn_shrubs` / `Disturbed.burn_grass` | Only present when `'disturbed' in ron.mods`; tie into SBS map availability |
| `btn_build_landuse` | button | Submit landuse build job | N/A | Triggers `POST /rq-engine/api/runs/<runid>/<config>/build-landuse` | ControlBase disables/enables button; ensure macros tag `data-command` for HTMX replacement |
...
| `input_upload_cli` | file (.cli) | Upload custom climate file | Local `.cli` file, validated by `Climate.set_user_defined_cli` | `POST /rq-engine/api/runs/<runid>/<config>/tasks/upload-cli/` | Only enabled when build mode = user-defined; macro needs to surface allowed extension |
...
| `btn_build_climate` | button | Submit climate build job | N/A | Triggers `POST /rq-engine/api/runs/<runid>/<config>/build-climate` (enqueues job, writes NoDb via `parse_inputs`) | JS manages state (`MODE_CONFIG`) and WebSocket lifecycle |
...
| Clip Hillslopes (`clip_hillslopes.htm`) | `clip_hillslopes` checkbox; `hillslope_clip_length` (m) | `api_run_wepp` updates the stored watershed choice and `.clip_hillslope_length`; WEPP prep applies the length per OFE | Multi-OFE total length may exceed the limit; shared-width scaling preserves complete hillslope area |
| Clip Soils (Maximum + Minimum) & Initial Saturation (`clip_soils_depth.htm`) | `clip_soils`; `clip_soils_depth` (mm); `clip_soils_minimum`; `clip_soils_minimum_depth` (mm); `initial_sat` (fraction) | `api_run_wepp` writes to `Soils.clip_soils`, `.clip_soils_depth`, `.clip_soils_minimum`, `.clip_soils_minimum_depth`, `.initial_sat` | Maximum remains backward-compatible legacy clipping; minimum extends shallow profiles; reject `clip_soils_minimum_depth > clip_soils_depth` when both toggles are enabled |
...
| `input_upload_landuse` | file (.img/.tif) | Upload treatment map | User-provided raster; validated & reprojected during build | `POST /rq-engine/api/runs/<runid>/<config>/build-treatments` (reuses `build_landuse_rq` pipeline) | Pure control uses `save_run_file` (100&nbsp;MB limit, `.tif/.img` allow-list); field hidden unless mode `4` selected |
...
| `btn_build_treatments` | button | Apply treatments | N/A | Calls `/rq-engine/api/runs/<runid>/<config>/build-treatments`, which triggers `build_landuse_rq` with treatment context | RQ job shares WS channel `treatments`; button wired via delegated handler |
...
| `btn_build_rap_ts` | button | Trigger RAP time-series acquisition | `POST /rq-engine/api/runs/<runid>/<config>/acquire-rap-ts` → enqueues worker job | ControlBase disables during readonly/lock; JS listens for completion to refresh status | `hint_build_rap_ts` announces success or errors via `aria-live` |
...
| `input_upload_sbs` | file (.tif/.img) | Upload Soil Burn Severity raster | `POST /rq-engine/api/runs/<runid>/<config>/tasks/upload-sbs/` → map saved/validated via `Baer.validate` | Accepts uint8/short thematic rasters; macros must note projection expectations |
| Upload/Remove buttons (`upload_sbs`, `remove_sbs`) | button | Manage SBS map | `Baer.upload_sbs()` / `remove_sbs()` call `tasks/upload-sbs` (via `/rq-engine/api`) and `tasks/remove_sbs` | Buttons hidden in readonly mode; expose `data-sbs-action="upload/remove"` so delegated handlers work in both legacy and Pure layouts |
...
- `disturbed_bp`: `/tasks/upload-sbs/` (via `/rq-engine/api`), `/tasks/remove_sbs/`, `/tasks/set_firedate/`, `/tasks/modify_burn_class`, etc.
...
| Contrast definition (`omni_contrasts_pure.htm`) | `omni_contrast_selection_mode` select; `omni_contrast_objective_parameter`, `omni_control_scenario`, `omni_contrast_scenario` selects; `omni_contrast_cumulative_obj_param_threshold_fraction` numeric; optional filters consumed by `Omni.parse_inputs` (`omni_contrast_hillslope_limit`, `omni_contrast_hill_min_slope`, `omni_contrast_hill_max_slope`, `omni_contrast_select_burn_severities`, `omni_contrast_select_topaz_ids`); scaffolded inputs: `omni_contrast_geojson` + `omni_contrast_geojson_name_key` (user-defined areas) and `order_reduction_passes` (stream-order pruning) | Posted to `/rq-engine/api/runs/<runid>/<config>/run-omni-contrasts`; `Omni.parse_inputs` maps values to enum fields/properties | Omni contrasts render as a dedicated section (`#omni-contrasts`) via `controls/omni_mod_pure.htm`; status stream uses `omni_contrasts`. Non-cumulative modes are scaffolded in UI and rejected by the backend until implemented. |
...
| Field ID | Input Type | Label / Purpose | Backend Binding | Visibility / Notes |
| --- | --- | --- | --- | --- |
| `btn_run_debris_flow` | button | Submit debris flow model | `POST /rq-engine/api/runs/<runid>/<config>/run-debris-flow` → enqueues `run_debris_flow_rq` | Panel gated to PowerUser (see template include); macros should warn results use Cannon 2010 model |
...
| `input_upload_*` (map uploads) | File inputs for ash maps | Posted to `/rq-engine/api/runs/<runid>/<config>/run-ash` when map files are attached (handled by `_task_upload_ash_map` + `save_run_file`) | Client-side + backend validation enforce <100&nbsp;MB and `.tif/.tiff/.img` extensions; Pure template passes data attributes for future enhancements |
...
| Batch Tasks | same | Task toggles, run button (`btn_run_batch`) | `BatchRunner.runBatch()` -> `/rq-engine/api/batch/_/<batch_name>/run-batch` | Layout currently uses Bootstrap cards; document for future redesign |
