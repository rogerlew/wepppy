...
- The `runs0` layout (`routes/run_0/templates/runs0_pure.htm`) composes the pure control partials, injects `controllers-gl.js`, and delegates layout to `ui-foundation.css` tokens (`wc-page`, `.pure-g`). Legacy `0.htm` remains in the repo but is no longer part of the production flow.【F:wepppy/weppcloud/routes/run_0/templates/runs0_pure.htm†L1-L120】
- JavaScript controllers remain singletons bundled into `controllers-gl.js`. `control_base.js` coordinates job polling, status/summary writers, stacktrace rendering, and StatusStream attachments across controls.【F:wepppy/weppcloud/controllers_js/control_base.js†L5-L199】
