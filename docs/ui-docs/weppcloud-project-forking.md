...
- `fork_rq(runid, new_runid, undisturbify, skip_wepp_runs_output)`:
  ...
  - **Approved; implementation pending (SURF-04A):** after rsync, normalize contract-listed legacy Omni child symlinks to
  ...
  - **Approved; implementation pending (SURF-04A):** report success only after recognized Omni links validate inside the
    destination run.
