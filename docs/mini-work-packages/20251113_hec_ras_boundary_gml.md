...
- Each GML contains one `gml:LineString` expressed in WGS84 (EPSG:4326). The line:
  - Passes through the center of the lowest raster pixel belonging to the channel.
  - Is orthogonal to the channel direction estimated from the two lowest pixels (lowest elevation/downstream distance).
...
- `Watershed.getInstance(wd)` already computed during DSS export; reuse it to fetch:
  ...
  - `relief` raster (same grid as `subwta`) to determine ordering by elevation; fallback to `discha` when relief unavailable.
...
2. Sample the matching cells in `relief` (or `discha` fallback) to rank them from lowest to highest value. “Lowest” means minimum elevation (preferred) or minimum downstream distance.
3. Extract the two lowest unique cells. If only one cell exists, synthesize the second point by walking upstream using `Watershed.flovec` or by picking the next-lowest hill pixel.
4. Convert raster indices `(col, row)` into projected coordinates using the GDAL transform:
   ```
   x = a + (col + 0.5) * b + (row + 0.5) * c
   y = d + (col + 0.5) * e + (row + 0.5) * f
   ```
5. Form the downstream direction vector `v = lowest - second_lowest`. Normalize it; the orthogonal vector is `n = (-v_y, v_x)` (rotate 90°).
...
Please review the scope/assumptions above before we begin implementation so we can resolve the open items and lock the geometry spec. Once agreed, we can proceed with the helper module, tests, and RQ wiring in a follow-up PR.
