...
> **Status:** Draft with partial implementation; setup discovery and orchestration-read endpoints are implemented (`/api/configs`, `/api/endpoints*`, `/api/runs/{runid}/{config}/pipeline`, `/api/runs/{runid}/{config}/readiness`), remaining controller-state surfaces are planned.
