...
\subsection{Why a discontinuity is plausible in the legacy calculation}

The Topanga Hill 106 instrumentation established two relevant implementation
features. First, WEPP accumulates soil-water surface return as a daily depth,
\surdra. The peak-flow path divides that depth by a selected positive-excess
duration and adds the resulting constant rate to selected subdaily intervals.
\Ksat{} can change both the returned depth and the selected duration. A larger
daily depth can therefore be compressed into a shorter interval even while
total runoff and maximum infiltration-excess rate decline.

Second, an exact-zero ponding-time sentinel selects the approximate peak method
(\texttt{APPMTH}), while a positive value selects characteristic routing
(\texttt{HDRIVE}). There is no continuity constraint between their returned
peaks. Forced-solver experiments show that this branch can enlarge a gap, but
the surface-return timing construction can produce a large gap even when both
lanes use the same solver.

The instrumented February 14, 1980 Hill 106 event illustrates the mechanism.
Changing only \Ksat{} from 20 to 35 mm/h reduced final
runoff from 60.122 to 58.630 mm, but increased the daily
surface-return depth from 36.999 to 51.637 mm. The selected
positive-excess duration shortened from 4,536.6 to 2,478.4 seconds. The imposed
surface-return rate consequently increased from 29.36 to 75.00 mm/h, and the
published peak increased from 47.710 to 92.714 mm/h.

This known case motivated a census rather than another isolated sensitivity
run. The census was designed to preserve full antecedent history and discover
where similarly abrupt responses occur across the complete Topanga hillslope
population.
...
identical scales, uses color for scenario, hollow triangle orientation for the
two-by-two mutation matrix, and opacity for directional agreement. Faint
...
\begin{figure}[H]
  ...
  Triangle orientation identifies mutation family and direction. Opacity
  identifies agreement with the ordinary inverse hydrologic response. The
...
\subsection{Chapter 4 expectations and historical context}

Stone et al. described peak discharge as one of four surface-hydrology
quantities supplied primarily to the WEPP erosion component, together with
rainfall-excess duration, rainfall intensity during excess, and runoff volume
\cite{stone1995}. For continuous simulation, most event peaks are not taken
from a retained, fully routed hydrograph. They are estimated with an
approximation to kinematic-wave routing. WEPP then assumes steady discharge at
that peak and computes an effective duration, $D_e=Q_v/q_p$, so that the
rectangular representation preserves event runoff volume in the rill-erosion
calculation. The reported peak is therefore an erosion-oriented reduction of
event hydrology as well as a hydrologic output; its interpretation is not
equivalent to that of an observed hydrograph ordinate.

Chapter 4 both supported and qualified this construction. Within the tested
dimensionless domain, the three branches of the continuous-simulation
approximation had a reported combined average error of 6.6\% relative to the
kinematic-wave routing solution. That was an approximation comparison, not a
population-scale validation against observed event peaks. Stone et al. also
warned that WEPP represents Hortonian flow but not variable contributing areas
or return flow explicitly, computes rainfall excess before routing, does not
fully couple routed water with recession infiltration in all cases, and
expects greater peak error for multipeaked hydrographs. Against those warnings,
the chapter made a clear favorable claim: assuming correct parameter values,
surface hydrology ``should perform the best for large events on a single OFE''
and the worst for medium events on multiple OFEs.

The Topanga mutation census empirically refutes that favorable expectation when
performance includes local event-scale reliability. The experiment operates at
hillslope scale, corresponding to the favorable single-OFE setting, yet the
incongruent share of central-band \Ksat{} responses increases rather than
decreases with baseline peak magnitude. The census also identifies rare
many-fold changes under one-percent probes while event runoff remains nearly
fixed. These are not merely the small-event noise, multiple-OFE averaging, or
multipeaked-hydrograph errors emphasized in the documented limitations. The
events most relevant to upper-tail interpretation are among those for which
the expected local response becomes less dependable.

The boundary of that refutation should remain explicit. Mutation stability is
not the same experiment as comparison with observed peaks, and baseline
\PeakRO{} is not necessarily identical to the rainfall-excess or runoff-volume
measure of event size that informed the original claim. The census therefore
does not erase the published routing comparisons. It does show that their
average-error result cannot support the broader inference that a large,
single-OFE peak is a dependable individual prediction.

The discrepancy is understandable in historical context. The original
developers evaluated the approximation with the theory, selected cases, data,
and computing resources available in the early 1990s. They did not claim to
have exhaustively tested local continuity under small parameter changes across
complete, multi-decadal event populations. A census comprising hundreds of
thousands of paired event responses would have been computationally
impractical during the original development effort. The Topanga census applies
modern computational capacity to a different validation dimension that appears
not to have been systematically examined in the intervening decades:
population-scale local response continuity. The result should therefore be
read as extending, rather than faulting, the original validation work while
still recognizing that one of its performance expectations does not survive
this new empirical test.
