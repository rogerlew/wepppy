...
Excluding mixed infiltration-excess and surface-return events reduces that
incongruent share at 30--100 mm/h from 24.1\% to 1.8\% unburned and from
26.0\% to 1.9\% burned, concentrating the directional instability in the
mixed-forcing population.
...
\subsection{The missing coupling and why a direct repair was abandoned}

The fundamental shortcoming is not simply that WEPP calculates saturation
excess. It is that the peak-flow path does not carry the calculated hourly
saturation-excess series forward as a time-varying source to the kinematic-wave
calculation. The hourly water balance knows when water is returned to the
surface, but the legacy path reduces those hourly values to the daily scalar
\surdra{}. After the original infiltration-excess event has been constructed,
the peak routine attempts to reconcile that missing runoff by dividing the
daily \surdra{} depth by a selected duration and adding the resulting constant
rate to the existing rainfall-excess array. The kinematic-wave solver, when
selected, therefore receives a reconstructed forcing series rather than the
actual hourly combination of infiltration excess and saturation excess. This
distinction is important: the addback occurs before the final \texttt{HDRIVE}
call, but after the model has discarded the timing needed to define the source
correctly. Volume can still balance for the day while the imposed rate, event
shape, and resulting \PeakRO{} depend on an artificial duration.

The production Papanicolaou (Lane D) water-balance implementation in
openWEPP takes a different approach. It retains a closing 24-bin surface-runoff
ledger and preserves saturation-return water in the hour in which the soil-water
calculation produced it. That same closed hourly ledger supplies the active
Lane D routing source. The published hillslope peak is defined more
conservatively as the maximum hourly mean flow from the ledger, rather than as
an instantaneous subhourly kinematic-wave peak. The complete frozen Topanga
small-mutation design was then repeated in openWEPP: all 1,088 trials completed,
with 1,913,158 finite paired-positive peak rows, and no row combined runoff
volume within 5\% of baseline with a peak change of twofold or greater. Thus,
the openWEPP study did not reproduce the unexplained, volume-stable \PeakRO{}
anomalies found in legacy WEPP. This is evidence for the benefit of preserving
hourly source timing, not a claim of observed-flow accuracy, subhourly peak
resolution, or watershed-outlet validation \cite{openweppclosure}.

A bounded WEPP-Forest repair was also attempted. Its intended change was to
retain hourly saturation excess separately, combine it with infiltration excess,
and let the existing characteristic solver route that time-varying lateral
inflow. The feasibility work was stopped before production wiring because the
existing solver could not accept the required series without changing deeper
contracts. In the mixed Hill 106 test, its normal 95\%-volume stopping rule
terminated at 75,600 seconds even though forcing continued through 86,400
seconds; only 93.58\% of the combined input had arrived by that time. In a
saturation-only test, exact zero-flow intervals caused a floating-point trap in
the characteristic integration, where cumulative terms are divided by the
current step rate. Retaining the legacy substitute of a tiny positive rate
would avoid the trap only by erasing the dry gaps that the repair was meant to
preserve. The attempted change also crossed established ownership boundaries
for infiltration, depression storage, recession infiltration, runoff volume,
event duration, and erosion inputs. It was therefore not a safe, local patch:
it would require a zero-aware characteristic formulation or another explicitly
time-series solver, followed by renewed water-balance, erosion, and routing
validation. The experimental code was removed and WEPP-Forest was restored
byte-for-byte to the diagnostic release; the failed feasibility evidence was
retained in the closed work package \cite{forestpf01}.
...
\subsection{Excluding mixed runoff sharply reduces directional incongruence}

Figure~\ref{fig:filtered-scatter} repeats Figure~\ref{fig:scatter} after
excluding pairs in which either the baseline or mutant uses forcing mode 1:
surface return is added to intervals already having positive infiltration
excess. In the census observer source, this branch requires realized surface
return (\texttt{surpls}) greater than $10^{-6}$ m and positive pre-addition
rainfall-excess duration (\texttt{durre}). The filter therefore isolates the
mixed Green--Ampt runoff and surface-return pathway implicated by the Hill 106
trace. It retains events without surface return and events with surface return
but no positive-excess duration; it is not a filter for all positive \surdra.
The peak floor, pairing rules, axes, markers, and opacity convention are
unchanged. Of 199,086 plotted rows, 115,932 remain: 56,356 unburned and 59,576
burned.

\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{artifacts/topanga-figure-3-excluding-mixed-runoff.pdf}
  \caption{Mutant versus baseline peak flow after excluding mixed
  infiltration-excess and surface-return forcing in either paired run.
  Green and orange markers remain because surface-return-only events are
  retained. Faint markers are directionally congruent; dark markers are
  incongruent. The filtered population is much more tightly aligned with
  the expected conductivity response, including at higher peak intensities.}
  \label{fig:filtered-scatter}
\end{figure}

The principal result is the sharp fall in the incongruent share of
\Ksat{} responses within the central 0.5--2 peak-ratio band
(Table~\ref{tab:filtered-incongruence}). At baseline peaks of 30--100 mm/h,
the share falls from 24.1\% to 1.8\% in the unburned stratum and from
26.0\% to 1.9\% in the burned stratum. At 10--30 mm/h, it falls from
7.4\% to 0.7\% unburned and from 18.5\% to 2.7\% burned. Thus, the
high-intensity directional instability in the full census is concentrated
in the mixed-forcing population. Among retained rows, the steep rise in
wrong-direction conductivity responses is largely absent.

\begin{table}[H]
  \centering
  \caption{Incongruent share of central-band \Ksat{} rows before and after
  mixed-forcing exclusion. Bins include their lower bound and exclude their
  upper bound; $n$ is the number of event rows in each denominator.}
  \label{tab:filtered-incongruence}
  \begin{tabular}{llrrrr}
    \toprule
    Stratum & Baseline peak & Original $n$ & Incongruent & Retained $n$ & Incongruent \\
     & (mm/h) & & & & \\
    \midrule
    Unburned & 10--30 & 5,540 & 7.4\% & 3,740 & 0.7\% \\
             & 30--100 & 10,399 & 24.1\% & 652 & 1.8\% \\
             & 100--300 & 3,254 & 33.4\% & 42 & 2.4\% \\
    Burned   & 10--30 & 12,596 & 18.5\% & 5,831 & 2.7\% \\
             & 30--100 & 21,149 & 26.0\% & 4,199 & 1.9\% \\
             & 100--300 & 1,466 & 23.1\% & 368 & 1.4\% \\
    \bottomrule
  \end{tabular}
\end{table}

This pattern supports the report's timing-based interpretation. Conductivity
changes affect both infiltration excess and profile return. In the mixed
branch, a change in positive-excess duration also changes the rate imposed
by dividing daily return depth by that duration. The filtered comparison
locates most high-intensity directional incongruence in the population exposed
to that coupling. It does not establish that duration compression causes
every excluded reversal; antecedent state and solver selection also vary
within this population and require frozen-event replay.

The extreme-ratio tail also contracts, but remains distinct from directional
incongruence. Exclusion removes 41.8\% of plotted rows and 69.5\% of rows
outside the 0.2--5 band. The retained population contains 425 rows outside
0.5--2 (0.367\%) and 184 outside 0.2--5 (0.159\%). All 184 remaining
fivefold departures have no positive surface return in either run; 144 are
burned and 169 are \Ksat{} mutations. The burned twofold-tail rate is nearly
unchanged (0.577\% before versus 0.574\% after filtering). Mixed-forcing
exclusion therefore explains the disappearance of much of the directional
pattern without resolving the separate no-return extreme population.

This is a descriptive exclusion analysis, not a corrected simulation. No
retained peak changes value. Larger events are disproportionately removed,
and only 42 unburned central-band \Ksat{} rows remain at 100--300 mm/h;
none remain at 300--800 mm/h in either stratum. The within-bin comparisons
reduce, but do not eliminate, differences in population composition. Event
rows remain clustered within trials and years. A tighter diagonal demonstrates
local response stability in the retained population, not agreement with
observed peaks or proof that excluded events are physically invalid.
...
The filtered comparison further locates the high-intensity directional
instability in mixed infiltration-excess and surface-return events. Excluding
that pathway in either paired run lowers central-band \Ksat{} incongruence
at 30--100 mm/h from approximately one in four rows to fewer than one in
50 in both strata. This supports the surface-return timing interpretation,
while the 184 retained fivefold departures without surface return demonstrate
that it does not explain every extreme response. Filtering selects a different
population; it does not repair the excluded peaks.
...
\begin{itemize}
  ...
  \item the completed openWEPP hourly peak-authority closure and repeated
    Topanga mutation census; and
  \item the closed WEPP-Forest feasibility package documenting why direct
    hourly saturation-return insertion was abandoned.
\end{itemize}
...
\bibitem{openweppclosure}
openWEPP Hourly Peak-Runoff Authority Closure (2026).
\path{/workdir/openWEPP/docs/work-packages/20260809-hourly-peak-runoff-authority-closure-001/}.

\bibitem{forestpf01}
WEPP-Forest PF01 \surdra{} Kinematic-Wave Partitioning Work Package (2026).
\path{/workdir/wepp-forest_260430_baseline/docs/work-packages/20260810-pf01-surdra-kinematic-wave-partitioning/}.
