...
- [x] (2026-08-04 06:09Z) Moved the proposed sole production consumer from
  wepp1 to otherwise-idle wepp3 and recorded the opt-in, dependency-minimal
  worker-only topology and preflight requirements.
- [ ] Register the bounded enhancement and ratify the exact contract matrix.
- [ ] Obtain and disposition two independent checkpoint reviews, then commit
  the checkpoint as a standalone ancestor and record its revision.
...
- Decision: Propose queue name `fork-archive`, service name
  ...
  policy.
  Date/Author: 2026-08-04 / Codex; exact operator approval pending.
...
- Decision: Run the sole production consumer on otherwise-idle wepp3, with
  ...
  Date/Author: 2026-08-04 / Roger Lew and Codex; exact compose mechanism remains
  subject to checkpoint review.
...
stack, `docker/docker-compose.prod.worker.yml`, is used on wepp2 and is the
proposed source of the opt-in wepp3 service. Because the production hosts share
...
proposed production implementation adds an opt-in service to
`docker/docker-compose.prod.worker.yml` and starts only that named service on
wepp3. It must not depend on `rq-worker`, `rq-worker-batch`, `f-esri`, or
...
follow each file's local pattern with count `1`, mount the same run volumes and
secrets as `rq-worker`, and use the native WEPPpyo3 interchange entrypoint.
...
console, and SURF-17 RQ info-details contracts. Under
...
as a bounded enhancement composing SURF-03, SURF-04, and SURF-17 without
advancing or reopening their unrelated behavior.

Amend the three concise owner contracts with the exact normative delta and mark
...
define one dedicated consumer and the wepp1 layered application stack does not
activate one. Prove the worker-only production compose defines an opt-in
one-process service consuming only `fork-archive`, with the required
entrypoint/volumes/secrets/startup gate and no dependency on the normal workers,
`f-esri`, or `weppcloudr`. Prove normal wepp2 activation excludes it while an
explicit wepp3 target starts only that service. Keep the repurposed HPC compose
a non-consumer. Update existing WEPPpyo3 and worker-startup contract tests where
the new service is a consumer.

Update wctl, job-listing, rq-engine Admin, and RQ info-details tests so default
queue order is `default`, `batch`, `fork-archive`. Add exact render assertions
...
and restore enqueue sites. Do not change response payloads, auth, timeout,
arguments, job metadata, archive markers, or downstream jobs.

Add `rq-worker-fork-archive` to dev and Forest using their adjacent worker
conventions and one worker process. Add an opt-in service to the worker-only
compose for wepp3, using its external Redis, secrets, image, and NFS mount
contracts but no unrelated service dependency. Do not activate the service on
wepp1 or wepp2. Render default and explicitly targeted compose configurations
...
Exercise queued cancellation through existing authorized surfaces where
available and confirm no destination/archive job marker is left stale. Exercise
...
For rollback, first restore the enqueue sites to `default` while keeping the
dedicated worker online. Let its queued/started/deferred/scheduled registries
drain, then stop/remove the dedicated service. Prove no job remains stranded on
`fork-archive`. Do not flush Redis DB 9 as a substitute for rollback evidence.
...
Render compose contracts without mutating running services:
    ...
    RQ_REDIS_URL=redis://127.0.0.1:6379/9 COMPOSE_PROFILES=fork-archive docker compose -f docker/docker-compose.prod.worker.yml config --quiet
...
Docker tests for dev/Forest one-worker topology, opt-in wepp3 activation, and
...
fails, revert the enqueue sites first but leave the worker service running so
already accepted `fork-archive` jobs can finish. Inspect queued, started,
deferred, and scheduled registries before removing the service. Never move jobs
between queues by editing raw Redis lists and never flush DB 9 merely to make a
...
it. No public Python API or response schema changes.

Dev and Forest must expose service `rq-worker-fork-archive` with one RQ worker
subscribed only to `fork-archive`. The worker-only compose must expose the same
service as opt-in for wepp3, using the production image, native interchange
entrypoint, external Redis DB 9 credentials, run mounts, and startup readiness
contract without dependencies on unrelated services. Wepp1 and normal wepp2
activation must not start it.
...
non-consumption, opt-in wepp3 isolation, wepp1/wepp2 non-consumption,
