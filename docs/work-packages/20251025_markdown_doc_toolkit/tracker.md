...
- [ ] **CI Integration Sprint (Nov 3)**
  ...
  - [x] Publish SARIF results to Code Scanning dashboard with failure gates on errors (Docs Quality workflow uploads). Formatter now emits camelCase fields so SARIF uploads succeed without post-processing; lint scope excludes `.docker-data` via explicit path list.
