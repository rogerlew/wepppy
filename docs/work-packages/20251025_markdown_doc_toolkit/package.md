...
- **CI Integration (Week of Nov 3):**
  ...
  - ✅ `docs-quality.yml` workflow on self-hosted runner now executes doc-lint/doc-bench (scoped to documentation paths so `.docker-data` stays excluded), uploads CodeQL-compliant SARIF directly, and conditionally runs Rust checks when `MARKDOWN_DOC_WORKSPACE` (or known workspace paths) expose a Cargo project; follow-up: set the env var on the runner so fmt/clippy/test always execute.
