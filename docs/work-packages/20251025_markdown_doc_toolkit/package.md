...
- **CI Integration (Week of Nov 3):**
  ...
  - ✅ `docs-quality.yml` workflow on self-hosted runner now executes doc-lint/doc-bench (scoped to documentation paths so `.docker-data` stays excluded), uploads CodeQL-compliant SARIF directly, and runs Rust checks when `MARKDOWN_DOC_WORKSPACE` is provided (sourced from repository secret in workflow env). Ensure homelab secrets stay current so fmt/clippy/test always execute.
