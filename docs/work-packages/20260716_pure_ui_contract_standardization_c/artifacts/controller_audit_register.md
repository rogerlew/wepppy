...
| `agFields` / DOM-13A..13D | `C/ag_fields.js`; `T/ag_fields_pure.htm` | `R/run_0/run_0_bp.py`, AgFields; `wepppy/rq/ag_fields_rq.py` stage/suite chain | DOM-13A verifies boundary/schema/subfield and DOM-13B plant/mapping actual render, browser, route/state and enqueue evidence; DOM-13C-D remain queued; high | contractual | partially verified |
| `wepp` / DOM-14A..14C | `C/wepp.js`; `T/wepp_pure.htm` plus WEPP/SWAT advanced partials | `R/nodb_api/wepp_bp.py`, Wepp/SWAT; WEPP stage pipeline | DOM-14A verifies core run/executable/lifecycle render plus browser, Flask, RQ-engine and queue evidence; DOM-14B-C remain queued; high | contractual | partially verified |
