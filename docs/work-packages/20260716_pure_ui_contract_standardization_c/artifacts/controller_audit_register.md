...
| `map` / DOM-04A/04B | `C/map_gl.js` + four `C/map_gl_*` helpers; `T/map_pure_gl.htm` | `R/nodb_api/watershed_bp.py` and public map/query routes; map state; no RQ expected | DOM-04A: actual render, coordinate/search/drilldown, exact elevation payload, service/report tests; DOM-04B layers/config matrix remains pending; low, public-route change high | contractual | DOM-04A verified; DOM-04B unverified |
