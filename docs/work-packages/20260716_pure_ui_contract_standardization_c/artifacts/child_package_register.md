...
| SURF-04B | `20260806_fork_skip_omni_reset` | SURF-04, SURF-04A, DOM-25A, DOM-25B | Add one default-false fork option; exclude exactly the two Omni scenario/contrast collection nodes; recreate empty real collections; rewrite only destination Omni persisted/artifact/cache/lock state after identity rewrite; remove exactly the two Omni RedisPrep timestamps; invalidate copied query-engine catalog/cache; strengthen checked-job readiness; preserve legacy jobs, unrelated timestamps/datasets/controllers, auth, source state, and queue topology | `high`: public fork input, RQ mixed-version boundary, run-tree omission/reset, NoDb/RedisPrep/query-catalog integrity | contract amendment review |
...
| SURF-04B | `20260806_fork_skip_omni_reset` | Add a default-false fork option that omits the two Omni child-project collection nodes, establishes fresh destination-only Omni controller/artifact state, removes only Omni RedisPrep completion markers, and invalidates copied query-engine catalog/cache before success | SURF-04, SURF-04A, DOM-25A, DOM-25B | High / `high`: route/RQ, filesystem reset, NoDb cache/locks, RedisPrep/query-catalog integrity | contract amendment review |
