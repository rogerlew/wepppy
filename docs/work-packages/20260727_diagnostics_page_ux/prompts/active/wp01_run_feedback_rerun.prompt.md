...
`/weppcloud/diagnostics/` runs all registered checks once at DOMContentLoaded (`page.js` invokes the core runner). While checks run — bandwidth probes with 10-second default timeouts and realtime probes with their own windows push total wall time past 30 seconds — the check list shows a single static "Running diagnostics..." placeholder that is only replaced when results settle. There is no progress indication and no way to re-run without reloading the page.
...
- `wepppy/weppcloud/static/js/diagnostics/bandwidth_checks.js`, `auth_checks.js`, `diagnostics-realtime.js` — check modules; they register with the core and should not need changes unless the runner cannot observe start/settle without them
- `wepppy/weppcloud/static-src/tests/smoke/diagnostics/` — Playwright smoke specs that may assert on the current placeholder markup
...
6. Extend the Jest suites; update Playwright smoke specs only if they assert on replaced markup.
