...
class OSMRoadsResult:
    ...
    stale_served: bool                           # True if stale entry served after upstream error
    fetched_at_utc: str                          # ISO timestamp of source fetch
    source: Literal["cache", "overpass", "stale_cache"]
    feature_count: int
    target_epsg: int
...
- Metadata index (SQLite): `<cache_root>/index.sqlite`
- Payloads (GeoParquet preferred): `<cache_root>/tiles/<tile_id>/<filter_hash>.parquet`
- Cross-process lock files: `<cache_root>/locks/<tile_key>.lock`
- SQLite is metadata/index only (not lock coordinator) and must run with WAL + busy-timeout pragmas.
...
- Expired (`age > hard_ttl`): must refresh; stale may only be served when explicitly enabled and documented.
- `force_refresh=True`: always attempt upstream refresh first (independent of freshness state). If refresh fails, stale fallback remains bounded by `allow_stale_on_error` and `hard_ttl`.

## Locking Contract

- Per-tile-key single-flight lock required (tile payload write granularity).
- Locking must be cross-process (file locks or equivalent OS-level lock), not in-process threading locks.
...
4. Read cached tile payloads for hits.
...
8. Merge tile payloads for request, clip merged roads to AOI, and reproject to `target_epsg`.
9. Materialize consumer-facing GeoJSON artifact in target CRS.
10. Return `OSMRoadsResult` with absolute `roads_geojson_path` in target CRS.
...
- `WEPPPY_OSM_ROADS_SQLITE_BUSY_TIMEOUT_MS` (default `5000`)
...
- Expired tile payloads and metadata rows (`age > hard_ttl`) must be eligible for deletion by module-owned cleanup.
...
- concurrency test proving single-flight fill,
