// Supply a short-lived run-scoped token on stdin; never print credentials.
...
(async () => {
  ...
  try {
    ...
    for (const relative of ['landuse/hill_101.mofe.man', 'wepp/runs/p1.man']) {
      ...
      const local = fs.readFileSync('/wc1/runs/mo/' + run + '/' + relative);
      ...
      console.log(JSON.stringify({ run, relative, pageStatus: response.status(),
        landuseFormVisible: true, downloadStatus: download.status(), sha256: sha(content) }));
    }
