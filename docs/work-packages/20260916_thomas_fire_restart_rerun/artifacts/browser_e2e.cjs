...
(async()=>{
 ...
 try {
  ...
  async function discover(suffix){
   const value=await page.evaluate(async u=>{const r=await WCHttp.requestWithSessionToken(u,{method:'GET'});return {status:r.status,body:r.body};},'/rq-engine/api/'+suffix);
   fs.writeFileSync(path.join(output,'discovery-'+suffix.replace(/[^a-zA-Z0-9_-]/g,'_')+'.json'),JSON.stringify(value,null,2));
   record({stage:'discovery',path:suffix,status:value.status});return value.body;
  }
  await discover('configs');await discover('endpoints');await discover('endpoints/rq_engine_create/schema');
  const prefix=`runs/${runid}/${config}/`;
  await discover(prefix+'pipeline');await discover(prefix+'readiness');
  const ops=await discover(prefix+'endpoints?include_operation_docs=true');
  record({stage:'operation_discovery',postfire_catalog_entries:JSON.stringify(ops).includes('postfire-debris-flow')});
  if(JSON.stringify(ops).includes('postfire-debris-flow')){
   for(const kind of ['schema','defaults','errors']) await discover(prefix+'endpoints/rq_engine_postfire_run/'+kind);
  }else{
   const response=await page.request.get(base+'/rq-engine/openapi.json');assert.equal(response.status(),200);
   const api=await response.json();
   const route=Object.entries(api.paths).find(([p])=>p.endsWith('/postfire-debris-flow/run'));
   assert(route);fs.writeFileSync(path.join(output,'postfire-openapi.json'),JSON.stringify(route,null,2));
   record({stage:'discovery_catalog_gap',action:'Use registered OpenAPI route and existing browser controller payload; no direct guessed mutation.'});
  }
  ...
  await discover(prefix+'pipeline');await discover(prefix+'readiness');await discover(prefix+'outputs');
