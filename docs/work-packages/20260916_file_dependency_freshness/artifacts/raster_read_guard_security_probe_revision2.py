...
def retain(name, result):
    Path(__file__).with_name(f"raster_read_guard_security_{name}_revision2.json").write_text(json.dumps(result, indent=2)+"\n")
...
def test_intermediate_native_generation_is_not_admitted(tmp_path, monkeypatch, kind):
    ...
    if kind == "bytes_restored":
        ...
        # Actual negative control: this native summary does not change for
        # the temporary PAM nodata declaration. Directory drift still rejects.
        assert observed[0] == expected
