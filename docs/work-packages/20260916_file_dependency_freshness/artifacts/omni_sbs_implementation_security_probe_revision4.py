...
def test_same_byte_new_upload_during_final_source_observation_is_preserved(tmp_path, monkeypatch):
    source = tmp_path / 'upload.tif'
    source.write_bytes(b'accepted uploaded bytes')
    target = tmp_path / 'child.tif'
    execution = fresh.SbsExecution.capture(definition(source))
    ...
    monkeypatch.setattr(fresh, 'sha256_file', digest)
    error = capture_error(lambda: execution.copy_to(target))
    record('same_byte_new_upload', {'error': error, 'source_retained': source.exists(),
                                    'initial_inode': initial_inode, 'new_inodes': replaced,
                                    'copied_bytes': target.read_text()})
    assert replaced and replaced[0] != initial_inode
    assert error is not None and error['errno'] == errno.ESTALE
    assert source.read_bytes() == target.read_bytes() == b'accepted uploaded bytes'
