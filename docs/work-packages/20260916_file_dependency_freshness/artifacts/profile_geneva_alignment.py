"""Retained actual C06 component profile after failed first acceptance run.
...
previous = json.loads((ARTIFACTS / 'geneva_implementation_performance.json').read_text())
...
try:
    with patch.object(Path, 'open', opened), \
        ...
        profile.disable()
        path = ARTIFACTS / 'geneva_alignment_hit_profile.txt'
        ...
        for _ in range(3):
            ...
            query()
            profile.disable()
        with (ARTIFACTS / 'geneva_alignment_miss_profile.txt').open('w') as outgoing:
    ...
    (ARTIFACTS / 'geneva_alignment_component_profile.json').write_text(payload)
