...
def landuse_symlink(root):
    sources = [root / name for name in (
        'wepp/output/interchange/loss_pw0.hill.parquet',
        'watershed/hillslopes.parquet', 'landuse/landuse.parquet')]
    write(sources[0], [{'wepp_id': 1, 'Runoff Volume': 100., 'Subrunoff Volume': 20.,
        'Baseflow Volume': 10., 'Soil Loss': 5., 'Sediment Yield': 3.,
        'Sediment Deposition': 2.}])
    write(sources[1], [{'wepp_id': 1, 'topaz_id': 101, 'area': 1000.}])
    write(sources[2], [{'topaz_id': 101, 'key': 100, 'desc': 'Forest'}])
    initial = AverageAnnualsByLanduseReport(root)
    expected = [dict(row.row) for row in initial]
    cache = root / 'wepp/reports/cache/average_annuals_by_landuse.parquet'
    target = cache.with_name('selected-report-storage.parquet')
    cache.rename(target)
    cache.symlink_to(target.name)
    assert [dict(row.row) for row in AverageAnnualsByLanduseReport(root)] == expected
    write(target, [{'older_schema': 1}])
    rebuilt = [dict(row.row) for row in AverageAnnualsByLanduseReport(root)]
    assert rebuilt == expected and cache.is_symlink()
    assert list(pd.read_parquet(target).columns) == initial.header
    return {'cache_symlink_preserved_on_read_and_rebuild': True,
            'resolved_target_contains_rebuilt_rows': True}
...
with TemporaryDirectory(prefix='reports-security-checkpoint-') as temporary:
    ...
    result = {'uid': os.getuid(), 'gid': os.getgid(),
              ...
              'landuse_destination': landuse_symlink(root / 'landuse-symlink')}
