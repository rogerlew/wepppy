"""Explicitly amended C05/C06 actual service/provenance/publication acceptance.

Corrects initial phase preparation: observe all dependencies before admission
wait. Original failed measurements and their diagnostic review remain retained.
...
OUTPUT = ARTIFACTS / 'geneva_implementation_performance_amended.json'
...
result['performance_contract'] = {'decision': 'geneva_performance_budget_amendment.md',
    'decision_sha256': hashlib.sha256((ARTIFACTS / 'geneva_performance_budget_amendment.md').read_bytes()).hexdigest(),
    'original_failures_retained': ['geneva_implementation_performance.json', 'geneva_implementation_performance_revision2.json'],
    'geometry_ms': {'settled_hit': 40, 'cold_evicted_hit': 75, 'paired_miss_added': 125},
    'burn_ms': {'settled_hit': 30, 'cold_evicted_hit': 40, 'paired_miss_added': 65}}
...
try:
    ...
    with patch.object(Path, 'open', counted_open), patch.object(freshness, 'observe_raster_dependencies', observe), \
        ...
        for label, query in [('geometry', geometry_query), ('burn', burn_query)]:
            query()  # Inputs may have been evicted by the preceding case.
            sleep(1.05)
            query()
    ...
    for label, settled_limit, cold_limit, miss_limit in [('geometry', 40, 75, 125), ('burn', 30, 40, 65)]:
