...
function queryParquet(payload) {
  return JSON.parse(execFileSync(python,['-c', 'import sys,json,duckdb; p=json.loads(sys.argv[2]); c=duckdb.connect(); c.execute("CREATE VIEW loss AS SELECT * FROM read_parquet(?)",[sys.argv[1]]); cur=c.execute("SELECT "+",".join(p["columns"])+" FROM loss ORDER BY wepp_id"); print(json.dumps({"records":[dict(zip([x[0] for x in cur.description],r)) for r in cur.fetchall()]}))',parquet,JSON.stringify(payload)],{encoding:'utf8'}));
}
function fixture() {
