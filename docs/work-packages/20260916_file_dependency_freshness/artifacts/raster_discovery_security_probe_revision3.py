...
@pytest.mark.parametrize("kind", ["gtiff_mask", "gtiff_overview"])
...
<DstRect xOff="0" yOff="0" xSize="4" ySize="4"/>
        ...
        if kind.startswith("gtiff_"):
            ...
            vrt.write_text(xml)
            suffix = ".msk" if kind == "gtiff_mask" else ".ovr"
            vrt.rename(Path(str(source) + suffix))
            vrt = source
        observations = []
        ...
        Path(__file__).with_name(f'raster_discovery_security_{kind}_revision3.json').write_text(json.dumps(output, indent=2) + '\n')
