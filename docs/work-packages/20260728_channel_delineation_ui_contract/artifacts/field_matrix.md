...
| Successful conditioning diagnostics (DOM-05B conformance pending) | no new submitted field; after successful channel delineation, the ordinary status panel renders one plain-text summary naming the conditioning method, maximum terrain raise, maximum terrain cut, and method-specific outcome; summary/detail are not duplicated and flat-routing increments remain separately attributed | each WBT method writes the exact sidecar in `docs/schemas/wbt-conditioning-diagnostics-contract.md`; the worker enforces identity, atomicity, validation, cleanup, job-meta, trigger, polling, and replay rules; invalid diagnostics use the specified controlled error | DOM-05B four-tool output parity, atomicity/confinement/parser, RQ trigger/poll/replay, mixed-version, and both-controller text-rendering regressions |
