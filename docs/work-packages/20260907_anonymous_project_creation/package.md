...
**Status**: Open — scaffolded, implementation not started (2026-09-07)
**Timezone**: UTC
...
Proposed environment variable: `WEPPCLOUD_ALLOW_ANONYMOUS_PROJECT_CREATION`, default `true`. Setting it to `false` opts into the restriction. Use the same setting and boolean interpretation in Flask and rq-engine. Finalize accepted values and invalid-value behavior in the pre-implementation contract checkpoint; invalid configuration must not silently enable anonymous access.
...
Follow `docs/standards/contract-first-change-standard.md`: prepare current canonical contract amendments, a starting revision and valid-state matrix, two independent read-only checkpoint reviews and disposition, and the required standalone checkpoint ancestor before implementation. No commits or implementation were requested during scaffolding.
...
- [ ] Unset/true preserves anonymous CAPTCHA creation and existing authenticated behavior.
- [ ] False rejects anonymous direct POSTs, including valid CAPTCHA, without creation side effects.
- [ ] False permits valid authenticated browser and API creation under existing permissions.
- [ ] Restricted anonymous interfaces HTML omits all creation controls and CAPTCHA assets; sign-in and informational content remain usable.
- [ ] Both service containers receive the same explicit value; operator enable/disable instructions and rollback are verified.
- [ ] Canonical contracts, Docker README/defaults documentation, and affected user/developer guides agree.
- [ ] Focused and required broad gates, real workflow evidence, and independent reviews are complete.

## Deliverables and References

- [Implementation plan](prompts/active/anonymous_project_creation_execplan.md)
- [Tracker](tracker.md)
...
Open. This delivery is a planning scaffold; implementation, wiring, validation, and rollout remain future work.
