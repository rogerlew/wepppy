...
Unburned (green) and burned (orange) hillslopes are shown on separate left and
right axes with identical scales, preventing either stratum from masking the
other. Hollow triangle orientation identifies the mutation: downward and
...
reference lines, colors, marker mapping, and opacity classification.
