...
| Bootstrap enable | `weppcloud/bootstrap/enable_jobs.py`; `wepppy/rq/wepp_rq.py::bootstrap_enable_rq` | Durable Bootstrap enable receipt and git-lock token | Run ID, exact enable operation/origin; the preallocated job ID is also the opaque lock token so a confirmed-missing ambiguous enqueue can owner-safely clear only its correlated stale lock; deferred receipts release their exact old token before replacement |
