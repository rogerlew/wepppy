#!/usr/bin/env python3
"""Capture or compare the immutable input trees for AgFields acceptance."""
...
SCHEMA = pa.schema(
    ...
    metadata={b"purpose": b"ag_fields_authoritative_preexisting_inventory_v1"},
)
...
def _files(root: Path) -> Iterator[Path]:
    ...
    while pending:
        ...
        for entry in entries:
            ...
            if entry.is_symlink():
                raise RuntimeError(f"Authoritative inventory contains a symlink: {path}")
            if entry.is_dir(follow_symlinks=False):
                pending.append(path)
            elif entry.is_file(follow_symlinks=False):
                yield path
            else:
                raise RuntimeError(f"Unsupported authoritative inventory entry: {path}")
...
def capture(run_root: Path, output: Path) -> dict[str, object]:
    ...
    for tree, relative_root in ROOTS:
        ...
        if not root.is_dir():
            raise FileNotFoundError(f"Missing authoritative tree: {relative_root}")
        tree_counts[tree] = 0
        tree_bytes[tree] = 0
    ...
    return {
        ...
        "runid": run_root.name,
        "roots": {tree: relative_root.as_posix() for tree, relative_root in ROOTS},
...
def main() -> int:
    ...
    parser.add_argument("--compare", nargs=2, type=Path, metavar=("EXPECTED", "OBSERVED"))
