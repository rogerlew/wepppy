...
The 2026-07-10 amendment applies the seeded soil/landuse generator's existing
profile boundary clamp to every mutation profile before serialization. The
baseline and edge profiles use the existing broad physical bounds; targeted
profiles retain their existing narrower bounds. This prevents independent
`wp` and `fc` multipliers from producing `wp > fc`, including in later OFEs of
multi-OFE seed soils. No bound or multiplier value changed.
...
Amendment Decision Venue: Codex operator session, 2026-07-10 17:45 PDT

Amendment Participants Present: Roger Lew, Codex

Amendment Decision Owner(s): Roger Lew / WEPPcloud maintainer

Amendment Implementer(s): Codex
...
- Every seeded mutation profile applies the already-defined horizon boundary
  clamp before writing, so independently mutated `wp`/`fc` values preserve the
  serializer contract across every OFE.
...
4. Repair only the failing multi-OFE fixture - rejected because the inversion
   is generated by production mutation logic and can affect any OFE/profile
   combination using independent `wp` and `fc` multipliers.
...
- Seeded generator regression: `tests/wepp/test_seeded_soil_landuse_generators.py::test_generate_case_preserves_multi_ofe_soil_structure`.
...
Keep the core guard local to SSURGO and WEPP soil utility code, and require
seeded generators to satisfy it before serialization. Do not alter the global
`isfloat()` helper for this incident. Production batch invalidation must remove
`build_soils` and downstream task timestamps only after fixed worker code is
deployed.
