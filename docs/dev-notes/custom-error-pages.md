...
## Current Findings
- `exception_factory` (`wepppy/weppcloud/utils/helpers.py:211`) always serializes a JSON payload with an `error` envelope (stack traces in `error.details`), even when HTML pages call it (for example `wepppy/weppcloud/routes/weppcloud_site.py:22`). The helper writes the stack trace to `<runid>/exception_factory.log`, but it does not negotiate content type with the caller.
...
4. **Adjust run routes**: Add explicit preflight checks in `runs0_nocfg` and `runs0` that collapse missing runs and unauthorized/private runs into a single 404 response with privacy-preserving copy (“This run is unavailable. It may be private or the link is incorrect.”). Update `authorize` or its callers to align with the new 404 behavior.
