...
* Upload traffic now routes through `/rq-engine/*` so Caddy can apply longer upstream timeouts and forward directly to the rq-engine FastAPI service.
...
* After reloading Caddy, wait for the pfSense/HAProxy health check to report green before hitting `/rq-engine` or `/weppcloud`; transient 502s are expected while HAProxy marks the backend healthy.
