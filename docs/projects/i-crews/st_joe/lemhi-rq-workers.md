...
**Status:** DevOps Thought Experiment

**Date:** 2026-04-15  

**Author:** Roger Lew, with AI support from GPT 5.4, Codex 5.3, Claude Opus 4.6, and Gemini 3.1 Pro  

**Audience:** WEPPcloud stakeholders, I-CREWS leadership, RCDS/Lemhi operations, security/network administrators  

**Purpose:** Demonstrate, through exhaustive technical planning, why running WEPPcloud rq-workers on Lemhi is not viable — and why every major research-computing center that encountered this class of problem built separate infrastructure instead.


> **Accuracy note:** Technical details pertaining to WEPPcloud architecture and Lemhi/C3+3 infrastructure reflect verified sources. External references are cited and were checked at time of authorship. Timeline and cost estimates were generated by Codex 5.3 and should not be used as a proper basis for engineering management decisions without independent estimation.

> **Note:** This document is a companion to [procurement-request.md](procurement-request.md), which makes the positive case for dedicated servers. This document makes the negative case by showing what the Lemhi path would actually require. The thoroughness of the plan is the argument against it.

<!-- toc -->
- [1. Problem Statement](#problem-statement)
- [2. Baseline Assumptions from Current WEPPcloud Architecture](#baseline-assumptions-from-current-weppcloud-architecture)
- [3. Definition of "First-Class Persistent RQ Workers"](#definition-of-first-class-persistent-rq-workers)
- [4. Hard Prerequisites (External Dependencies)](#hard-prerequisites-external-dependencies)
- [5. Target Reference Architecture](#target-reference-architecture)
  - [5.1 Control Plane (WEPPcloud side)](#51-control-plane-weppcloud-side)
  - [5.2 Worker Plane (Lemhi side)](#52-worker-plane-lemhi-side)
  - [5.3 Storage Plane](#53-storage-plane)
...
- [6. Blocker Resolution Plan](#blocker-resolution-plan)
  ...
  - [6.3 Blocker C: Slurm scheduler model versus persistent workers](#63-blocker-c-slurm-scheduler-model-versus-persistent-workers)
    ...
    - [6.3.3 CPU allocation and `ulimit` contract (currently not documented in this repo)](#633-cpu-allocation-and-ulimit-contract-currently-not-documented-in-this-repo)
  - [6.4 Blocker D: Filesystem and small-file I/O mismatch](#64-blocker-d-filesystem-and-small-file-io-mismatch)
    - [6.4.1 WEPP FORTRAN workload: concrete file I/O profile](#641-wepp-fortran-workload-concrete-file-io-profile)
    - [6.4.2 St. Joe basin: file count at scale](#642-st-joe-basin-file-count-at-scale)
    - [6.4.3 Why this workload destroys Lustre performance](#643-why-this-workload-destroys-lustre-performance)
    - [6.4.4 Mandatory high-performance 3-2-1 storage procurement and integration](#644-mandatory-high-performance-3-2-1-storage-procurement-and-integration)
  - [6.5 Blocker E: Shared filesystem contract between Lemhi workers and WEPPcloud services](#65-blocker-e-shared-filesystem-contract-between-lemhi-workers-and-weppcloud-services)
  - [6.6 Blocker F: Docker-coupled worker tasks (for example `weppcloudr`)](#66-blocker-f-docker-coupled-worker-tasks-for-example-weppcloudr)
- [7. Workstreams and Deliverables](#workstreams-and-deliverables)
  - [7.1 DevOps Workstream](#71-devops-workstream)
  - [7.2 Software Engineering Workstream](#72-software-engineering-workstream)
  - [7.3 System Administration Workstream](#73-system-administration-workstream)
  - [7.4 Security Workstream](#74-security-workstream)
  - [7.5 Operations/SRE Workstream](#75-operationssre-workstream)
- [8. Required Software Backlog (Concrete)](#required-software-backlog-concrete)
  - [8.1 Software Backlog Accounting (Included in Baseline)](#81-software-backlog-accounting-included-in-baseline)
  - [8.2 Software Backlog Addendum (Currently Under-Accounted)](#82-software-backlog-addendum-currently-under-accounted)
- [9. Program Phases, Timeline, and Gate Reviews](#program-phases-timeline-and-gate-reviews)
- [10. Staffing Model (Minimum Practical Team)](#staffing-model-minimum-practical-team)
- [11. Cost Envelope (Order-of-Magnitude)](#cost-envelope-order-of-magnitude)
...
- [14. Practical Decision Framing](#practical-decision-framing)
- [15. Prior Art and Industry Evidence](#prior-art-and-industry-evidence)
  - [15.1 HPC Centers Explicitly Recommend Against This Pattern](#151-hpc-centers-explicitly-recommend-against-this-pattern)
  - [15.2 Major HPC Centers Built Separate Infrastructure Instead](#152-major-hpc-centers-built-separate-infrastructure-instead)
  - [15.3 Attempts to Run Task Queues on HPC Are Documented as Broken](#153-attempts-to-run-task-queues-on-hpc-are-documented-as-broken)
  - [15.4 Lustre Small-File Performance Is Well-Documented](#154-lustre-small-file-performance-is-well-documented)
  - [15.5 Security Precedent: HPC SSH Tunnels Have Been Weaponized](#155-security-precedent-hpc-ssh-tunnels-have-been-weaponized)
  - [15.6 Direct Database Connections Across HPC Perimeters Are Generally Prohibited](#156-direct-database-connections-across-hpc-perimeters-are-generally-prohibited)
...
- [16. Detailed Phase Execution Specs](#detailed-phase-execution-specs)
  - [16.1 Phase 0: Preconditions (6 Weeks, Apr 20-May 29, 2026)](#161-phase-0-preconditions-6-weeks-apr-20-may-29-2026)
    ...
    - [Explicit decisions and deadlines](#explicit-decisions-and-deadlines)
    - [Weekly plan](#weekly-plan)
    - [Acceptance criteria](#acceptance-criteria)
    - [Complexity (1-5)](#complexity-1-5)
  - [16.2 Phase 1: Platform Plumbing (8 Weeks, Jun 1-Jul 24, 2026)](#162-phase-1-platform-plumbing-8-weeks-jun-1-jul-24-2026)
    - [Objective](#objective)
    - [Scope boundaries](#scope-boundaries)
    - [Explicit decisions and deadlines](#explicit-decisions-and-deadlines)
    - [Weekly plan](#weekly-plan)
    - [Acceptance criteria](#acceptance-criteria)
    - [Complexity (1-5)](#complexity-1-5)
  - [16.3 Phase 2: Software Adaptation (Baseline 12 Weeks, Jul 27-Oct 16, 2026)](#163-phase-2-software-adaptation-baseline-12-weeks-jul-27-oct-16-2026)
    - [Objective](#objective)
    - [Entry criteria](#entry-criteria)
    - [Queue-routing design decisions](#queue-routing-design-decisions)
    - [Epic-level backlog and complexity](#epic-level-backlog-and-complexity)
    - [Story backlog (execution units)](#story-backlog-execution-units)
    - [Weekly milestones (baseline 12 weeks)](#weekly-milestones-baseline-12-weeks)
    - [Acceptance gates](#acceptance-gates)
    - [Rollback criteria and actions](#rollback-criteria-and-actions)
  - [16.4 Phase 3: Pilot Operations (8 Weeks, Oct 19-Dec 11, 2026)](#164-phase-3-pilot-operations-8-weeks-oct-19-dec-11-2026)
    - [Objective](#objective)
    - [Pilot design](#pilot-design)
    - [Soak and chaos scenarios](#soak-and-chaos-scenarios)
    - [SLO targets and abort thresholds](#slo-targets-and-abort-thresholds)
    - [Phase 3 complexity (1-5)](#phase-3-complexity-1-5)
  - [16.5 Phase 4: Production Hardening (6 Weeks, Dec 14, 2026-Jan 22, 2027)](#165-phase-4-production-hardening-6-weeks-dec-14-2026-jan-22-2027)
    - [Objective](#objective)
    - [Hardening design](#hardening-design)
    - [Hardening drills](#hardening-drills)
    - [Production SLO thresholds](#production-slo-thresholds)
    - [Phase 4 complexity (1-5)](#phase-4-complexity-1-5)
  - [16.6 Cross-Phase Complexity and Governance Summary](#166-cross-phase-complexity-and-governance-summary)
    - [Phase complexity totals](#phase-complexity-totals)
...
- [17. Blocker Option Evaluation and Viable Paths](#blocker-option-evaluation-and-viable-paths)
  ...
  - [17.6 Blocker F: Docker-coupled worker tasks](#176-blocker-f-docker-coupled-worker-tasks)
  - [17.7 Viable integrated paths](#177-viable-integrated-paths)
  - [17.8 Options not viable for near-term St. Joe delivery](#178-options-not-viable-for-near-term-st-joe-delivery)
  - [17.9 Security and Best-Practice-Backed Go/No-Go Decisions (Recommended Blockers)](#179-security-and-best-practice-backed-gono-go-decisions-recommended-blockers)
  - [17.10 Conditional Scope Adders (Decision-Dependent Work)](#1710-conditional-scope-adders-decision-dependent-work)
- [18. End Note: Long-Term Lemhi-First Modernization Path](#end-note-long-term-lemhi-first-modernization-path)
  - [18.1 Long-Term Work Packages (Post-Path A)](#181-long-term-work-packages-post-path-a)
  - [18.2 Conditional Governance for Long-Term Work](#182-conditional-governance-for-long-term-work)
<!-- tocstop -->
...
The answer is a multi-domain program touching software architecture, DevOps, cluster operations, security, networking, data management, and support ownership. The scope and complexity documented here should be read as evidence that this path is not a reasonable alternative to the $42K dedicated-server procurement.
...
**Additionally:** Conditional feature files (`frost.txt`, `phosphorus.txt`, `gwcoeff.txt`, `firedate.txt`, `pmetpara.txt`) are written per-run depending on active feature flags.
...
1. **NERSC (DOE)** built **Spin**, a separate Kubernetes-based platform for science gateways, databases, API endpoints, and workflow managers. NERSC's Science Gateways documentation cautions that "NERSC is not a web hosting service" and that developers "should not anticipate availability approaching what is available in commercial offerings"; formal support for science gateway services is 8x5. Persistent WEPPcloud-like workloads run on Spin, not on Perlmutter compute nodes.
   - Reference: <https://docs.nersc.gov/services/spin/>
   - Reference: <https://docs.nersc.gov/services/science-gateways/>

2. **University of Virginia Research Computing** maintains a separate clustered Kubernetes environment with ">1000 cores and ~1TB of memory" specifically for persistent containerized services (web servers, Shiny apps, APIs, databases), plus over 300 TB of cluster storage. Architecturally distinct from their HPC batch cluster.
