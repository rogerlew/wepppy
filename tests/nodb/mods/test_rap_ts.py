...
class RAP_TS(NoDbBase):
    ...
    def _post_instance_loaded(cls, instance):
        ...
        parquet_path = _join(instance.rap_dir, 'data.parquet')
        ...
        with self.locked():
            ...
            if pd is not None and pyarrow is not None and data_ds:
                ...
                if records:
                    ...
                    parquet_path = _join(self.rap_dir, 'data.parquet')
                    df.to_parquet(parquet_path)
                    
                # Set self.data to None before saving to avoid writing to nodb file,
                # then restore it for the live instance.
                _data = self.data
                self.data = None
                self.save()
                self.data = _data
