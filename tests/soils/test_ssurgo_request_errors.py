...
def test_make_soap_request_ssl_error_raises_user_message(monkeypatch) -> None:
    ...
    with pytest.raises(ssurgo.SsurgoRequestError) as excinfo:
        ssurgo._makeSOAPrequest("select 1")

    assert str(excinfo.value) == ssurgo._SSURGO_UNAVAILABLE_MESSAGE


def test_make_soap_request_timeout_retries_raise_user_message(monkeypatch) -> None:
