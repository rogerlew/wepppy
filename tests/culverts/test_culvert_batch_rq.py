...
def test_culvert_batch_retries_stale_runner_state_when_recording_job_ids(
    ...
    monkeypatch.setattr(CulvertsRunner, "_load_run_ids", lambda self, path: ["1"])
    monkeypatch.setattr(
        culvert_rq_module._pipeline,
        "enqueue_culvert_batch_jobs",
        lambda *a, **k: (_DummyJob("final-job"), {"1": "child-job-1"}),
    )

    lock_attempts = {"count": 0}

    @contextmanager
    def _stale_once_locked(*args, **kwargs):
        del args, kwargs
        lock_attempts["count"] += 1
        if lock_attempts["count"] == 2:
            raise NoDbStaleWriteError("simulated stale write")
        yield

    monkeypatch.setattr(runner, "locked", _stale_once_locked)
    ...
    assert runner._runs["1"]["job_id"] == "child-job-1"
