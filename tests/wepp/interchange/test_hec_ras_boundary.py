...
def test_build_boundary_condition_features_creates_geojson_and_gml(tmp_path, monkeypatch):
    ...
    subwta = np.array([[0, 1114], [0, 1114]], dtype=np.int32)
    metric = np.array([[10.0, 2.0], [9.0, 1.0]], dtype=np.float64)
    ...
    assert len(coords) == 2
    span = ((coords[0][0] - coords[1][0]) ** 2 + (coords[0][1] - coords[1][1]) ** 2) ** 0.5
    ...
    assert len(pos_text.strip().split()) == 4
