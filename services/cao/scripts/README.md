...
The legacy `make prod-workers-*` shortcuts are removed. Use
`docker/prod-worker-deploy-guide.md` for the supported worker deployment flow.
...
- For remote workers, follow `docker/prod-worker-deploy-guide.md` and ensure
  `docker/.env` exists on each host with `UID/GID` aligned to forest1 to avoid permission drift.
