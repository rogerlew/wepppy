---
title: "A report generated for `r (function(runid, config) { path <- if (!is.null(config) && nzchar(config)) sprintf('/weppcloud/runs/%s/%s', runid, config) else sprintf('/weppcloud/runs/%s', runid); sprintf('<a href=\"%s\" style=\"color: #6a4c93; text-decoration: none;\">%s</a>', path, runid) })(params$proj_runid, params$proj_config)` run"
date: "`r Sys.Date()`"
output: 
...
`r if(sum(runoff_cp_df$cum_runoff_mm, na.rm = TRUE)==0) {"<!--"}`
...
`r if(sum(runoff_cp_df$cum_runoff_mm, na.rm = TRUE)==0) {"<span class='big_txt'>No runoff was generated during the entire simulation.</span>"}`
