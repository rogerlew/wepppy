...
usage() {
  cat <<'USAGE' >&2
Usage:
  ./wctl/install.sh [dev|hpc|prod|wepp1|wepp3|worker]
...
case "${ENVIRONMENT}" in
  dev)
    ...
    ;;
  hpc)
    COMPOSE_RELATIVE_PATH="docker/docker-compose.dev.hpc.yml"
    COMPOSE_EXTRA_RELATIVE_PATH=""
    ;;
  prod)
  ...
  wepp1)
    ...
    ;;
  wepp3)
    COMPOSE_RELATIVE_PATH="docker/docker-compose.prod.wepp3.yml"
    COMPOSE_EXTRA_RELATIVE_PATH=""
    ;;
  worker)
