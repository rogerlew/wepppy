...
- Prefer the smallest reversible fix that can plausibly resolve the confirmed failing path; treat theoretical limitations as acceptance-test risks, not proof of failure. Before adding a queue, service, datastore, daemon, dependency, privilege, protocol, or deployment topology, require retained evidence that the simpler approach missed the real acceptance condition and record the smallest escalation plus its operational/recovery cost. An operator-selected bounded solution authorizes testing that solution, not silent redesign. For mechanical refactors, preserve code structure (scope/indent/order) and change only the intended tokens.
