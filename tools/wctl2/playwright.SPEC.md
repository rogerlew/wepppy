...
| `--run-path` | String | _(none)_ | Reuse existing run path (skips provisioning) |
| `--create-run / --no-create-run` | Flag | `True` | Auto-provision runs via `/tests/api/create-run` |
...
| Option | Type | Default | Description |
|--------|------|---------|-------------|
| `--playwright-args` | String | _(none)_ | Additional Playwright CLI arguments (raw passthrough) |
...
| `--create-run` | `SMOKE_CREATE_RUN` | `process.env.SMOKE_CREATE_RUN` |
...
def register(app: typer.Typer) -> None:
    ...
    def run_playwright(
        ...
        if ui:
            pw_args.append("--ui")
        if report:
            pw_args.append("--reporter=html")
        
        if playwright_args:
            pw_args.extend(playwright_args.split())
        ...
        if report and result.returncode == 0:
            report_cmd = ["npm", "run", "playwright", "show-report"]
