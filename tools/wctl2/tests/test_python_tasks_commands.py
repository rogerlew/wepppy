...
def test_check_rq_graph_runs_guard_script(monkeypatch: pytest.MonkeyPatch, temp_project) -> None:
    runner = CliRunner()
    recorded: List[Tuple[str, str, bool, bool]] = []
    ...
    assert recorded == [
        (
            "weppcloud",
            "cd /workdir/wepppy && /opt/venv/bin/python tools/check_rq_dependency_graph.py",
            True,
            False,
        )
    ]
