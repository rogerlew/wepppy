"""CLIGEN runner, station metadata helpers, and localization utilities."""
...
def _query_station_db(
    db_path: str, bbox: Tuple[float, float, float, float] | None
) -> Tuple[list[tuple[Any, ...]], list[tuple[Any, ...]]]:
    """Return station + state rows from the requested SQLite catalog.

    Args:
        db_path: Absolute path to the SQLite database generated by the station
            builder.
        bbox: Optional bounding box `[ul_x, ul_y, lr_x, lr_y]`. When provided
            the query filters both latitude and longitude against the box.

    Returns:
        Two lists of tuples: rows from the `stations` table followed by the
        associated `states` lookup rows.
    """
...
def _load_fallback_station_data(
...
def _make_clinp(
    ...
    cli_fname: str,
    par: str,
    clinp_fn: str = "clinp.txt",
) -> str:
    """Write the ``clinp`` control file consumed by CLIGEN binaries."""
...
def is_date(year: int, mo: int, da: int) -> bool:
    """Return True when the provided year/month/day is a valid calendar date."""
    try:
...
def df_to_prn(
    ...
    Args:
        df: Input dataframe indexed by date.
        prn_fn: Destination path for the ``.prn`` file.
        p_key: Column name containing precipitation in millimetres.
        tmax_key: Column name for daily maximum temperature in Celsius.
...
class Prn:
    """Wrapper around CLIGEN ``.prn`` files that exposes pandas transformations."""

    def __init__(self, fn: str):
        """Load a CLIGEN ``.prn`` file into a pandas dataframe.

        Args:
            fn: Path to the ``.prn`` file generated by CLIGEN.

        Notes:
            The file encodes precipitation in hundredths of an inch and
            temperatures in Fahrenheit; callers are responsible for unit
            conversions.
        """
    ...
    @property
    def prcp_key(self):
        """Return the dataframe column name for precipitation values."""
        return self.header[3]

    @property
    def tmax_key(self):
        """Return the dataframe column name for daily maximum temperature."""
        return self.header[4]

    @property
    def tmin_key(self):
        """Return the dataframe column name for daily minimum temperature."""
        return self.header[5]

    def get_outlier_mask(self, column):
        """Return a boolean mask that flags 1.5×IQR outliers for ``column``."""
        df = self.df
    ...
    def replace_outliers(self, other):
        """In-place replacement for precipitation/temperature outliers.

        Args:
            other: Another ``Prn`` instance or dataframe whose aligned values
                will replace the detected outliers. Records without a matching
                replacement fall back to CLIGEN's ``9999`` sentinel.
        """
    ...
    def write(self, fn=None):
        """Serialize the dataframe back to CLIGEN ``.prn`` format."""
...
class ClimateFile(object):
    """Parser/utility wrapper for CLIGEN ``.cli`` files."""

    def __init__(self, cli_fn):
        """Parse a CLIGEN ``.cli`` file and cache its metadata lines.

        Args:
            cli_fn: Path to the CLIGEN output file that should be parsed.
        """
    ...
    def is_single_storm(self) -> bool:
        """Return True when the CLI file only contains one rainfall event."""
    ...
    def last_date(self) -> datetime.date:
        """Return the final date represented in the CLI file."""
    ...
    def clip(self, start_date: datetime.date, end_date: datetime.date):
        """Trim the CLI records to the inclusive ``start_date``/``end_date`` window.

        Args:
            start_date: First day to keep.
            end_date: Last day to keep.
        """
    ...
    @property
    def years(self):
        """Return the sorted list of year values contained in the file."""
        df = self.as_dataframe()
    ...
    def is_observed(self):
        """Return True when the dataset only contains historic (pre-1900) data."""
        years = self.years
        y0 = min(years)
        return y0 > 1900
    ...
    @property
    def is_future(self):
        """Return True when the dataset extends beyond the current calendar year."""
        years = self.years
        yend= max(years)
    ...
    def input_years(self):
        """Return the number of input years represented by the CLI file."""
        df = self.as_dataframe()
        years = [int(v) for v in sorted(set(df['year']))]
        return max(years) -  min(years) + 1

    def make_storm_file(self, dst_fn):
        """Write a legacy WEPP storm file that mirrors the CLI contents.

        Args:
            dst_fn: Destination file path for the ASCII storm listing.
        """
        warnings.warn(
...
"""
    ...
    def as_dataframe(self, calc_peak_intensities=False):
        """Return the CLI contents as a pandas dataframe.
    ...
    def count_wetdays(self):
        """Return the monthly average number of wet days across the dataset."""
        df = self.as_dataframe()
    ...
    def calc_monthlies(self):
        """Return monthly precipitation and temperature summaries in inches/°F."""
        df = self.as_dataframe()
        nyears = len(set(df.year))
    ...
    def calc_intensity(self):
        """Return percentile-based precipitation intensity diagnostics."""
    ...
    def write(self, fn):
        """Write the current line buffer to ``fn``."""
        with open(fn, 'w') as fp:
            fp.write(''.join(self.lines))

    @property
    def contents(self):
        """Return the entire CLI file as a single string."""
        return ''.join(self.lines)


class Station:
    """Structured access to CLIGEN ``.par`` metadata and localization helpers."""
    def __init__(self, par_fn):
        """Parse a `.par` file and expose the monthly statistics arrays."""
    ...
    def monthly_ppts(self):
        """Return precipitation totals per month (inches)."""
    ...
    def write(self, fn):
        """Write the in-memory PAR lines to ``fn``."""
        with open(fn, 'w') as fp:
            fp.write(''.join(self.lines))
...
class StationMeta:
    ...
    def __init__(self, state, desc, par, latitude, longitude, years, _type,
        ...
        Args:
            state: Two-letter state or region code.
            desc: Human-readable station description.
            par: Path or filename for the station's ``.par`` file.
            latitude: Station latitude in decimal degrees.
            longitude: Station longitude in decimal degrees.
            years: Span of record years contained in the station files.
            _type: Station classification flag emitted by CLIGEN.
            elevation: Elevation in metres (may be missing).
            tp5: TP5 metadata column from the station table.
            tp6: TP6 metadata column from the station table.
            annual_ppt: Average annual precipitation in inches.
            _distance: Optional pre-computed distance populated by callers.
        """
    ...
    def get_station(self):
        """Return a ``Station`` loader bound to this metadata record."""
        return Station(self.parpath)

    def calculate_lat_distance(self, loc_lat):
        """Populate ``lat_distance`` using the provided query latitude."""
        self.lat_distance = abs(self.latitude - loc_lat)

    def calculate_distance(self, location):
        """Populate ``distance`` using the provided ``(lng, lat)`` tuple."""
        self.distance = haversine(location, (self.longitude, self.latitude))

    def as_dict(self, include_monthlies=False):
        """Return a JSON-serializable representation of the station.

        Args:
            include_monthlies: When True, embed monthlies from the backing
                `.par` file (requires reading the file from disk).
        """
        d = {
            "state": self.state,
            "desc": self.desc,
    ...
    def __repr__(self):
        """Return a debug-friendly representation with all metadata."""
        return "%s(%r)" % (self.__class__, self.__dict__)

    def build_ghcn_daily_climate(self, prn_fn, cli_fn):
        """Construct `.prn`/`.cli` artifacts from GHCN daily observations.

        Args:
            prn_fn: Destination path for the synthesized ``.prn`` file.
            cli_fn: Destination path for the synthesized ``.cli`` file.
        """
...
class CligenStationsManager:
    """Loads station catalogs and exposes distance/heuristic search utilities."""
    def __init__(
        self, version: Union[str, int, None] = None, bbox: Sequence[float] | None = None
    ):
        """Load station metadata from the requested catalog.
    ...
    def get_closest_station(self, location):
        """Return the closest station to the provided ``(lng, lat)`` tuple."""
        self.order_by_distance_to_location(location)
        return self.stations[0]

    def get_closest_stations(self, location, num_stations):
        """Return ``num_stations`` nearest stations to ``location``."""
        self.order_by_distance_to_location(location)
    ...
    def get_stations_in_state(self, state):
        """Return all stations whose state/region code matches ``state``."""
    ...
    def get_closest_stations_by_lat(self, location, num_stations):
        """Return ``num_stations`` nearest stations using latitude distance."""
        self.order_by_lat_distance_to_location(location)
        return self.stations[:num_stations]

    def get_station_fromid(self, _id):
        """Return the first station whose `.par` filename contains ``_id``."""
        for station in self.stations:
    ...
    def get_stations_eu_heuristic_search(self, location, elev, pool=40):
        """Rank EOBS stations using precipitation and temperature agreement."""
    ...
    def get_stations_au_heuristic_search(self, location, elev, pool=None):
        """Rank Australian stations via AGDC monthlies and elevation similarity."""
    ...
    def export_to_geojson(self, geojson_fn):
        """Write the station collection to GeoJSON, falling back to /tmp on error."""
...
class Cligen:
    """Run CLIGEN binaries for multi-year, localized, or observed simulations."""
    def __init__(self, station: StationMeta, wd: str = './', cliver: str = "5.3.2"):
        """Configure a CLIGEN run context.

        Args:
            station: `StationMeta` describing the source `.par`.
            wd: Working directory where `.par/.cli/.log` artifacts are written.
            cliver: CLIGEN binary tag (`5.2`, `5.3`, `5.3.2`, or `4.3`).
        """
    ...
    def run_multiple_year(
        ...
        Args:
            years: Number of synthetic years to simulate.
...
def _tail(s: str, n: int = 80) -> str:
    """Return the last ``n`` lines from a log string."""
    return "\n".join(s.splitlines()[-n:])


def _run_cligen_posix(
    cmd: Sequence[str], clinp_path: str, timeout_sec: int, log_fp
) -> tuple[str, str]:
    """Invoke CLIGEN under Linux with aggressive timeout handling.

    Args:
        cmd: Full command list passed to ``subprocess``.
        clinp_path: Input file path whose contents are piped to stdin.
        timeout_sec: Number of seconds to wait before terminating the process.
        log_fp: File-like object used to append diagnostic information.

    Returns:
        Tuple of decoded stdout/stderr text produced by the CLIGEN binary.
    """
...
def par_mod(
    ...
    Args:
        par: Station identifier (matches entries in the SQLite catalog).
        years: Number of synthetic years to simulate.
        lng: Longitude for localization.
        lat: Latitude for localization.
        wd: Working directory where `.par/.cli` artifacts are generated.
        monthly_dataset: Source for monthly means (`prism`, `eobs`, `agdc`).
        nwds_method: ``""`` (use station probabilities) or ``"daymet"``.
        randseed: Optional CLIGEN random seed override.
        cliver: CLIGEN version tag; defaults to 5.3.
