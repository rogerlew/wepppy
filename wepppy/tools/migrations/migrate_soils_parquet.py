...
def _discover_run_dirs(root: Path) -> Iterable[Path]:
    ...
    for soils_dir in root.rglob("soils"):
        if soils_dir.is_dir():
            yield soils_dir.parent
...
def _parse_args(argv: Iterable[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Standardize soils parquet identifier columns.")
    parser.add_argument("--root", type=Path, default=Path("/wc1/runs"))
    parser.add_argument("--contains", action="append", default=[])
    parser.add_argument("--limit", type=int, default=None)
    parser.add_argument("--dry-run", action="store_true")
    parser.add_argument("--log-file", type=Path, default=Path("migrate_soils.log"))
        ...
        for idx, run_dir in enumerate(run_dirs, start=1):
            ...
            try:
                ...
                if args.dry_run:
                    ...
                    df.to_parquet(soils_fn, index=False)
                    status = "migrated"
                    migrated += 1
                else:
                    status = "unchanged"
