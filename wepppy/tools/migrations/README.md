...
python -m wepppy.tools.migrations.run_migrations --list-migrations
```

The runner is also available via the **Run Sync Dashboard** (admin-only) which can optionally run migrations after syncing a remote run.

### RQ Task

Migrations can be triggered as an RQ background task via `wepppy.rq.migrations_rq`:
...
The `runner` module provides programmatic access to individual migrations:

```python
from wepppy.tools.migrations.runner import run_all_migrations, MigrationResult

# Run all migrations
result: MigrationResult = run_all_migrations("/wc1/runs/bi/biogeographic-six")
...
result = run_all_migrations(wd, on_progress=on_progress)
```

## Available Migrations

The following migrations are run in order by the unified runner:
...
| `interchange` | Generate WEPP interchange Parquet files from text outputs |

All migrations are **idempotent** and safe to run multiple times. They will skip if no changes are needed.

## Standalone Scripts

The following standalone scripts are available for batch processing across multiple runs:
