...
- **Observed enqueue tracing (optional).** Set `WEPPPY_RQ_TRACE_ENQUEUE=1` to append runtime enqueue traces, then run `python tools/export_rq_observed_graph.py --write` to refresh `wepppy/rq/job-dependency-graph.observed.json`.
