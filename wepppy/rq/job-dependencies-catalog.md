...
- line `163` queue `default` stage `untracked` enqueue `func` depends_on *(none)*

### `wepppy/microservices/rq_engine/ag_fields_routes.py::_enqueue_watershed_jobs`
- line `244` queue `default` stage `untracked` enqueue `run_ag_fields_watershed_rq` depends_on `Dependency(jobs=[previous_job_id], allow_failure=True)`
...
The helper `wepppy.microservices.rq_engine.ag_fields_routes._enqueue_job` is represented as one dynamic `func` edge in the generated graph. Its three concrete call sites have no RQ `depends_on` edges; prerequisite checks happen synchronously before enqueue. The separate `_enqueue_watershed_jobs` edge expands `all` in Concept 1, Concept 2, hybrid order. Each job after the first depends on its predecessor through `Dependency(..., allow_failure=True)`, which prevents overlapping watershed memory peaks while permitting later comparisons after an earlier failure.
