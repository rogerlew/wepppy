...
- line `143` queue `default` stage `untracked` enqueue `func` depends_on *(none)*
...
The helper `wepppy.microservices.rq_engine.ag_fields_routes._enqueue_job` is represented as one dynamic `func` edge in the generated graph. Its three concrete call sites have no RQ `depends_on` edges; prerequisite checks happen synchronously before enqueue.
...
| `agfields_run_watershed` | `wepppy.rq.ag_fields_rq.run_ag_fields_watershed_rq` | Current successful Stage 4; observed climate; complete parent prepared inputs; writes only `wepp/ag_fields/watershed/` |
