...
- line `469` queue `default` stage `untracked` enqueue `run_ag_fields_watershed_rq` depends_on `Dependency(jobs=[previous_job_id], allow_failure=True)`
- line `495` queue `default` stage `untracked` enqueue `finalize_ag_fields_watershed_suite_rq` depends_on `Dependency(jobs=[child_job.id for child_job in scheme_jobs], allow_failure=True)`
...
| `agfields_run_wepp` | `wepppy.rq.ag_fields_rq.run_ag_fields_wepp_rq` | Current sub-fields; complete valid mapping; parent `wepp/runs/p<wepp_id>.sol` and `.cli` pairs |
