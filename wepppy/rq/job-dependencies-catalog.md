...
- Extracted enqueue edges: `138`
- Unique enqueue targets: `88`
...
- line `315` queue `default` stage `untracked` enqueue `target_func` depends_on *(none)*
...
- line `554` queue `default` stage `untracked` enqueue `build_landuse_rq` depends_on *(none)*

### `wepppy/microservices/rq_engine/landuse_routes.py::modify_landuse_mapping`
- line `813` queue `default` stage `untracked` enqueue `modify_landuse_mapping_rq` depends_on *(none)*
...
- line `784` queue `batch` stage `untracked` enqueue `delete_omni_contrasts_rq` depends_on *(none)*

### `wepppy/microservices/rq_engine/omni_routes.py::_run_omni`
- line `524` queue `batch` stage `untracked` enqueue `run_omni_scenarios_rq` depends_on *(none)*

### `wepppy/microservices/rq_engine/omni_routes.py::_run_omni_contrasts`
- line `648` queue `batch` stage `untracked` enqueue `run_omni_contrasts_rq` depends_on *(none)*
...
- line `82` queue `default` stage `untracked` enqueue `build_rusle_rq` depends_on *(none)*

### `wepppy/microservices/rq_engine/soils_routes.py::build_soils`
- line `122` queue `default` stage `untracked` enqueue `build_soils_rq` depends_on *(none)*
...
- line `968` queue `default` stage `untracked` enqueue `build_subcatchments_and_abstract_watershed_rq` depends_on *(none)*
...
- line `816` queue `default` stage `untracked` enqueue `set_outlet_rq` depends_on *(none)*
...
- line `1019` queue `default` stage `jobs:0` enqueue `build_subcatchments_rq` depends_on *(none)*
- line `1027` queue `default` stage `jobs:1` enqueue `abstract_watershed_rq` depends_on `build_subcatchments_rq`

### `wepppy/rq/project_rq.py::fetch_dem_and_build_channels_rq`
- line `779` queue `default` stage `jobs:0` enqueue `build_channels_rq` depends_on *(none)*
- line `794` queue `default` stage `jobs:0` enqueue `fetch_dem_rq` depends_on *(none)*
- line `802` queue `default` stage `jobs:1` enqueue `build_channels_rq` depends_on `fetch_dem_rq`

### `wepppy/rq/project_rq.py::fork_rq`
- line `1684` queue `default` stage `untracked` enqueue `_finish_fork_rq` depends_on `final_wepp_job`
...
- line `920` queue `default` stage `jobs:6` enqueue `sys.modules[__name__]._log_complete_rq` depends_on *(none)*

### `wepppy/rq/wepp_rq.py::run_wepp_watershed_rq`
- line `773` queue `default` stage `jobs:6` enqueue `sys.modules[__name__]._log_complete_rq` depends_on *(none)*
...
- line `275` queue `default` stage `untracked` enqueue `delete_run_rq` depends_on *(none)*

### `wepppy/weppcloud/routes/nodb_api/project_bp.py::task_set_readonly`
- line `627` queue `default` stage `untracked` enqueue `set_run_readonly_rq` depends_on *(none)*
