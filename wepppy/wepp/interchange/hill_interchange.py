...
def run_wepp_hillslope_interchange(wepp_output_dir: Path | str, *, ebe_start_year: int | None = None) -> Path:
    base = Path(wepp_output_dir)
    if not base.exists():
        raise FileNotFoundError(base)

    run_wepp_hillslope_pass_interchange(base)
    run_wepp_hillslope_ebe_interchange(base, start_year=ebe_start_year)
