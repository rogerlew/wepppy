...
def _augment_hill_channel_frames(
    ...
    if "wepp_id" not in hill_df.columns:
        columns = ", ".join(hill_df.columns)
        raise KeyError(f"Hillslope loss table missing wepp_id column; columns={columns}")

    if "chn_enum" not in chn_df.columns:
        columns = ", ".join(chn_df.columns)
        raise KeyError(f"Channel loss table missing chn_enum column; columns={columns}")

    hill_ids = pd.to_numeric(hill_df["wepp_id"], errors="coerce").astype("Int64")
    chn_ids = pd.to_numeric(chn_df["chn_enum"], errors="coerce").astype("Int64")
