...
def _get_subs_summary(parquet_fn, return_as_df=False):
    with duckdb.connect() as con:
        if return_as_df:
...
def get_soil_subs_summary(wd, return_as_df=False):
    parquet_fn = _join(wd, 'soils/soils.parquet')
    return _get_subs_summary(parquet_fn, return_as_df=False)
...
def get_watershed_chns_summary(wd, topaz_id):
    parquet_fn = _join(wd, 'watershed/channels.parquet')
    return _get_subs_summary(parquet_fn, topaz_id)
