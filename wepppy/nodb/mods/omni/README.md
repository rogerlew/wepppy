...
Omni groups hillslopes into contrast runs using a selection mode. Cumulative objective-parameter, user-defined areas, user-defined hillslope groups, and stream-order pruning (WBT-only) modes are implemented today.
...
#### User-defined hillslope groups (textbox)

- Users paste groups of hillslope Topaz IDs into a multiline textbox; each row becomes a contrast group.
- Parsing is tolerant of commas, whitespace, or both; trailing semicolons are allowed. Empty rows are ignored.
- Tokens must parse as integers. IDs are validated against the watershed translator (skip 0 and channel IDs ending in 4).
- Hillslope IDs may appear in multiple groups.
- Duplicate IDs within a single row are allowed and do not require special handling.
- Advanced filters (min/max slope, burn severity, select-topaz list) are ignored for this mode.
- Groups that end up empty after validation are skipped with `skip_status.reason = "no_hillslopes"`.
- Cumulative-only fields (objective parameter, cumulative threshold, hillslope limit) are ignored for this mode.
- User-defined hillslope groups require `contrast_pairs`; each pair expands across every group (N pairs x M rows). Duplicate pairs are ignored.
- Control/contrast scenario selectors are ignored in this mode (use the contrast pair list builder).
- `build_report.ndjson` rows include `selection_mode = "user_defined_hillslope_groups"`, `group_index`, `topaz_ids`, `n_hillslopes`, and `status` when skipped.
- Contrast ID overlays are generated by `_build_contrast_ids_geojson` using the group Topaz ID sets.
- Contrast IDs are stable across rebuilds by signature `control|contrast|group_index` while iterating groups in row order and expanding across contrast pairs in their configured order.
...
- **Selection source**: `build_report.ndjson` rows with `group_index` and `topaz_ids`.
- **Geometry**: union hillslope polygons from `Watershed.subwta_shp` (WGS GeoJSON) for each group.
- **Labels**: `contrast_label = group_index` (1-based row index).
...
- **User-defined list builder**: `contrast_pairs` is required for `user_defined_areas` and `user_defined_hillslope_groups` (duplicates deduped; N pairs × M polygons/rows, stable IDs for areas by `control|contrast|area_label`).
- **User-defined hillslope groups**: `omni_contrast_hillslope_groups` supplies row-based Topaz ID groups; validation applies, advanced filters are ignored, and empty groups are marked skipped.
...
- **Selection modes**:
  ...
  - User-defined hillslope groups: `contrast_id`, `control_scenario`, `contrast_scenario`, `group_index`, `n_hillslopes`, `skip_status`, `run_status`
...
**Cause**: Cumulative `build_contrasts()` filters by objective parameter threshold and optional constraints (slope, burn severity, topaz IDs)
