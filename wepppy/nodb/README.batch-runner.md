...
When climate resync changes a leaf, Batch Runner clears these timestamps only: `build_climate`, `build_rap_ts`, `build_openet_ts`, `run_wepp_hillslopes`, `run_wepp_watershed`, and `run_omni_scenarios`. DEM, watershed, landuse, and soils timestamps remain valid because climate settings do not change those artifacts.
