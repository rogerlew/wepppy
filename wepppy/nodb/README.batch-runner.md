...
| `run_watar` | on | Run WATAR ash transport and AshPost when the Ash mod is enabled |
...
### Base Project Resync and Leaf Invalidation

Each leaf run is cloned from `_base/` the first time it is initialized. If an operator fixes critical base climate settings after a leaf already exists, the retry classifier compares the leaf `climate.nodb` against `_base/climate.nodb`. Drift makes that leaf retry eligible even if its previous task timestamps otherwise look complete.

The current resync contract copies climate configuration fields that affect generated climate files or climate-dependent time series: mode, spatial mode, station-selection policy, catalog/user-defined CLI selection, observed/future year bounds, input years, CLIGEN database, observed quality guard toggles, GRIDMET wind behavior, and precipitation scaling settings. When the base policy is `FindClosestAtRuntime` with no station selected, a leaf climate build legitimately persists its resolved station and changes the leaf mode to `Closest`; that resolved pair is per-leaf runtime state and is not base-project drift. An explicit station or station-mode change in the base remains configuration drift. Generated climate outputs and per-leaf runtime state such as `cli_fn`, `par_fn`, `monthlies`, `sub_cli_fns`, `_cligen_seed`, and station search caches are not copied; they are regenerated by rerunning the affected tasks.

When climate resync changes a leaf, Batch Runner clears these timestamps only: `build_climate`, `build_rap_ts`, `build_openet_ts`, `run_wepp_hillslopes`, `run_wepp_watershed`, `run_watar`, and `run_omni_scenarios`. DEM, watershed, landuse, and soils timestamps remain valid because climate settings do not change those artifacts.

### WATAR and AshPost

`run_watar` is optional per leaf: it participates in completion and retry classification only when the cloned run contains `ash.nodb`. It runs after both WEPP hillslope and watershed timestamps exist and is not supported for single-storm runs. Before invoking the persisted `Ash` controller, Batch Runner repairs the hillslope and watershed interchange products and requires `H.pass.parquet`, `H.wat.parquet`, and `totalwatsed3.parquet`.
