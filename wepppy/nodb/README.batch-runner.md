...
The current resync contract copies climate configuration fields that affect generated climate files or climate-dependent time series: mode, spatial mode, station selection, catalog/user-defined CLI selection, observed/future year bounds, input years, CLIGEN database, observed quality guard toggles, GRIDMET wind behavior, and precipitation scaling settings. Generated climate outputs and per-leaf runtime state such as `cli_fn`, `par_fn`, `monthlies`, `sub_cli_fns`, `_cligen_seed`, and station search caches are not copied; they are regenerated by rerunning the affected tasks.
