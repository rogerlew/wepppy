# WEPPcloud Query Engine

## Overview

The WEPPcloud Query Engine is a **DuckDB-powered analytical service** that exposes watershed modeling outputs as queryable datasets. It transforms Parquet files, GeoJSON, and other structured data formats into a unified query interface, enabling users to analyze landuse, soils, climate, and WEPP model results without writing raw SQL or managing database connections.

### What It Does

- **Declarative Querying**: Submit JSON payloads describing filters, joins, aggregations, and computed columns; the engine generates optimized SQL and returns results
- **Schema Discovery**: Automatically catalogs available datasets with type information, making data exploration straightforward
...
- **No arbitrary SQL**: Users cannot submit raw SQL queries; all queries must be expressed through the JSON payload schema
- **No write operations**: The query engine is read-only; it does not modify datasets or accept INSERT/UPDATE/DELETE operations
...
│                                                             │
│  ┌───────────────┐  ┌──────────────┐  ┌─────────────────┐   │
│  │   Starlette   │  │   Payload    │  │    Executor     │   │
│  │   Web Server  │→ │  Validation  │→ │   (DuckDB)      │   │
│  │  (app/server) │  │ (payload.py) │  │ (executor.py)   │   │
│  └───────────────┘  └──────────────┘  └─────────────────┘   │
│         ↓                   ↓                    ↓          │
│  ┌───────────────┐  ┌──────────────┐  ┌─────────────────┐   │
│  │   Routes:     │  │   Query      │  │   Formatter     │   │
│  │   /query      │  │   Planner    │  │  (formatter.py) │   │
│  │   /catalog    │  │  (core.py)   │  │   - Schema      │   │
│  │   /activate   │  │   - Builds   │  │   - Records     │   │
│  │   /console    │  │     SQL      │  │   - Reshape     │   │
│  └───────────────┘  └──────────────┘  └─────────────────┘   │
│         ↓                                        ↑          │
│  ┌───────────────────────────────────────────────────────┐  │
│  │          Catalog (catalog.py)                         │  │
│  │  - catalog.json (dataset inventory + schemas)         │  │
│  │  - Generated by activate_query_engine()               │  │
│  └───────────────────────────────────────────────────────┘  │
│                                                             │
│  ┌───────────────────────────────────────────────────────┐  │
│  │          Run Context (context.py)                     │  │
│  │  - Resolves run directory from runid                  │  │
│  │  - Loads catalog, validates access                    │  │
│  └───────────────────────────────────────────────────────┘  │
└─────────────────────────────────────────────────────────────┘
                              ↓
                   ┌───────────────────────┐
...
| Module | Purpose |
|--------|---------|
| `app/server.py` | Starlette application with HTTP routes |
| `payload.py` | JSON payload validation and normalization (`QueryRequest`, `DatasetSpec`, `JoinSpec`, etc.) |
| `core.py` | SQL generation (`build_query_plan`) and query execution (`run_query`) |
| `executor.py` | DuckDB connection management and SQL execution with error handling |
| `formatter.py` | Result serialization (Arrow → JSON, schema extraction, timeseries reshaping) |
| `catalog.py` | Dataset catalog loading and schema lookup (`DatasetCatalog`, `CatalogEntry`) |
| `context.py` | Run directory resolution and catalog access (`RunContext`) |
| `activate.py` | Catalog generation from run directory contents |
| `app/query_presets.py` | Curated example queries organized by category |

### Request Flow

1. **HTTP Request** → User submits JSON payload to `/query-engine/runs/{runid}/{config}/query`
2. **Route Handler** → `app/server.py::run_query_endpoint()` receives request
3. **Context Resolution** → `context.resolve_run_context(runid, scenario=...)` locates run directory and loads `catalog.json`
4. **Payload Validation** → `payload.QueryRequest` dataclass validates and normalizes the JSON structure
5. **Query Planning** → `core.build_query_plan(payload, catalog)` generates DuckDB SQL with proper joins, filters, and aggregations
6. **Execution** → `executor.DuckDBExecutor.execute(sql)` runs the query in an isolated connection
7. **Formatting** → `formatter.format_table(arrow_table)` converts Arrow Table to JSON with schema metadata
8. **Response** → JSON payload returned with `records`, `schema`, `sql` (if requested), and error details (if failed)

### Deployment: Caddy Reverse Proxy Configuration

The query-engine runs as a standalone Starlette service (default port 8041) behind Caddy. The Caddy configuration routes `/query-engine/*` requests to the service:
...
**Key points:**
- The `handle_path` directive strips `/query-engine` from the path before forwarding
- `X-Forwarded-Prefix` header tells the service its external mount point
- Requests to `https://host/query-engine/runs/{runid}/{config}/query` reach the service at `/runs/{runid}/{config}/query`
- The service accepts only the documented path patterns; extraneous path segments after `/query` return 400 errors
...
|-------|--------|---------|
| `/runs/{runid}/{config}/query` | POST | **Execute a query** - Submit JSON payload, receive records + schema |
| `/runs/{runid}/{config}/catalog` | GET | **Browse catalog** - List available datasets with schema metadata |
| `/runs/{runid}/{config}/activate` | POST | **Activate catalog** - Generate `catalog.json` from run directory contents |
...
| Route | Method | Purpose | Scope Required |
|-------|--------|---------|----------------|
| `/mcp/ping` | GET | Health check | none |
| `/mcp/runs/{runid}` | GET | Run metadata | `runs:read` |
| `/mcp/runs/{runid}/activate` | POST | Trigger activation | `runs:activate` |
| `/mcp/runs/{runid}/catalog` | GET | Catalog with filtering | `runs:read` |
| `/mcp/runs/{runid}/presets` | GET | Example queries | `runs:read` |
| `/mcp/runs/{runid}/prompt-template` | GET | LLM prompt template | `runs:read` |
...
Before querying, a run must be **activated** to generate the `catalog.json` file:

1. **Trigger**: POST to `/runs/{runid}/{config}/activate`
2. **Discovery**: `activate.activate_query_engine(run_dir)` walks the run directory
3. **Schema Extraction**: For each Parquet file, read Arrow schema; for GeoJSON, infer from first features
4. **Filtering**: Exclude internal files (`.mypy_cache`, `_query_engine/`, `ash/` outputs)
...
Queries are **stateless** and **ephemeral**:
- Each request creates a new DuckDB connection
- No query history or session state is maintained
- Connections close immediately after query completion
- Parquet files are read directly from disk (no intermediate database)
...
## Querying Omni Scenarios

Omni runs support multiple treatment scenarios (e.g., mulch applications, undisturbed baselines). Each scenario stores its WEPP outputs in a subdirectory under `_pups/omni/scenarios/{scenario_name}/`. The query-engine resolves these paths automatically when you provide the `scenario` parameter.

### Scenario Query Structure

Pass the scenario **name** (not path) in the request body:

```json
{
  "scenario": "mulch_30_sbs_map",
  "datasets": ["wepp/output/interchange/loss_pw0.hill.parquet"],
  "columns": ["\"Hillslope Area\"", "\"Soil Loss\"", "\"Runoff Volume\""],
  "limit": 100
}
```

**How it works:**
1. The `scenario` parameter is extracted from the request body
2. `resolve_run_context(run_path, scenario="mulch_30_sbs_map")` resolves to `{run_path}/_pups/omni/scenarios/mulch_30_sbs_map/`
3. Dataset paths are resolved relative to the scenario directory
4. The scenario directory must contain its own activated catalog (`_query_engine/catalog.json`)

### Comparing Base vs Scenario Results

To compare base run against a scenario, make two separate requests:

**Base run query** (no scenario parameter):
```bash
curl -X POST "https://host/query-engine/runs/{runid}/{config}/query" \
  -H "Content-Type: application/json" \
  -d '{
    "datasets": ["wepp/output/interchange/loss_pw0.hill.parquet"],
    "columns": ["SUM(\"Soil Loss\") AS total_soil_loss"],
    "limit": 1
  }'
```

**Scenario query** (with scenario parameter):
```bash
curl -X POST "https://host/query-engine/runs/{runid}/{config}/query" \
  -H "Content-Type: application/json" \
  -d '{
...
### JavaScript Example (gl-dashboard pattern)

The gl-dashboard uses `postQueryEngineForScenario()` which extracts the scenario name from paths like `_pups/omni/scenarios/{name}`:

```javascript
// Base query (no scenario)
const baseResult = await postBaseQueryEngine({
  datasets: [{ path: 'wepp/output/interchange/loss_pw0.hill.parquet', alias: 'loss' }],
  columns: ['loss."Soil Loss" AS soil_loss_kg'],
});

// Scenario query (scenario extracted from path and added to body)
const scenarioPath = '_pups/omni/scenarios/mulch_30_sbs_map';
const scenarioResult = await postQueryEngineForScenario({
  datasets: [{ path: 'wepp/output/interchange/loss_pw0.hill.parquet', alias: 'loss' }],
  columns: ['loss."Soil Loss" AS soil_loss_kg'],
}, scenarioPath);
// Internally adds { scenario: 'mulch_30_sbs_map' } to the request body
```

### Invalid URL Patterns

**Do NOT** append scenario paths to the URL. The following patterns are **incorrect** and will return 400 errors:

```
❌ /query-engine/runs/{runid}/{config}/_pups/omni/scenarios/mulch_30/query
❌ /query-engine/runs/{runid}/{config}/scenario/mulch_30/query
```

The **correct** approach is always to use the standard URL and pass `scenario` in the request body:

```
✅ POST /query-engine/runs/{runid}/{config}/query
   Body: { "scenario": "mulch_30_sbs_map", "datasets": [...] }
```
