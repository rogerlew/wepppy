...
## Querying Scenario Subdirectories

The query-engine supports scenario-scoped catalogs under multiple run layouts,
including Omni and RHESSys. This is a **backend contract** in
`resolve_run_context(...)` and is independent of specific clients such as
GL Dashboard or Storm Event Analyzer.

### Scenario Query Structure

Pass `scenario` in the request body. Supported forms:

- Bare scenario name (for example: `"mulch_30_sbs_map"` or `"S1"`).
  Bare names resolve in this order:
  1. `{run_root}/_pups/omni/scenarios/{name}`
  2. `{run_root}/rhessys/scenarios/{name}`
- Explicit relative path from run root (for example:
  `"rhessys/scenarios/S1"`, `"_pups/omni/scenarios/undisturbed"`,
  `"omni/scenarios/undisturbed"`).

Example:

```json
{
  "scenario": "rhessys/scenarios/S1",
  "datasets": ["basin.daily.parquet"],
  "limit": 100
}
```

Each scenario directory must contain (or be able to auto-activate) its own
`_query_engine/catalog.json`.

### Comparing Base vs Scenario Results

To compare base run vs scenario, make separate requests:

**Base run query** (no `scenario`):
```bash
...
Do not append scenario paths to the URL. Use the standard route and pass
`scenario` in the request body.

Incorrect:
```
❌ /query-engine/runs/{runid}/{config}/rhessys/scenarios/S1/query
...
✅ POST /query-engine/runs/{runid}/{config}/query
   Body: { "scenario": "rhessys/scenarios/S1", "datasets": [...] }
```
