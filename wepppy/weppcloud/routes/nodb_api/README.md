...
- **Lookup schema note**: base lookup uses legacy scalar keys `rdmax`/`xmxlai`; extended lookup uses normalized keys `plant.data.rdmax`/`plant.data.xmxlai` plus `ini.data.*` and `plant.data.*` management fields.
...
| `/runs/<string:runid>/<config>/report/channel` | `GET` | Ron.map, Watershed.wbt_fill_or_breach | Renders `reports/channel.htm`; revalidates the run-scoped WBT sidecar and presents its measures as labeled Terrain Conditioning Diagnostics fields below the Summary |
