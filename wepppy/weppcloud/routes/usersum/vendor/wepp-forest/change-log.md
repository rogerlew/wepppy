...
| --- | --- | --- | --- | --- |
| 04/26/2026 | `44e0967` | `gfortran` | `wepp_260426` | Closed ablation campaign `20260426_taken-brainstem_p1408_hillslope_sigfpe-watbal-log10`; added `watbal` hk-domain hardening (`fc/ul <= 0 -> hk=0`) with observability markers (`WB_HK_FCUL`, `WB_HK_RATIO`, `WB_G_HKDOM`) to contain `p1408` `SIGFPE/log10`; published `release/wepp_260426` + `release/wepp_260426_hill` from pinned `/usr/bin/gfortran` with hashes `25e7fd49...` and `31b21c5b...`; validated host smoke gates (`src/wepp`, `src/wepp_hill`), full `pytest` (`43 passed`), reconciled-condenser watershed replay success marker, and ELF interpreter compatibility (`/lib64/ld-linux-x86-64.so.2`). |
