...
| --- | --- | --- | --- | --- |
| 04/20/2026 | `d9bb2e8` | `gfortran` | `wepp_260421` | Released `wepp_260421` and `wepp_260421_hill` with `/usr/bin/gfortran` via `tools/build_wepp_dated_release.sh`; increased hillslope `mxplan` guardrail from 15 to 20 (with inline do-not-exceed comments in hill include variants); validated host smoke gates, full `pytest`, reconciled-condenser manual watershed replay, and ELF interpreter compatibility (`/lib64/ld-linux-x86-64.so.2`). |
