...
That re-instrumentation surfaced five real defects: four that the legacy code had carried for years, and one in the new process kernels that was caught by the dual-basis gate before release promotion.
...
5. A **rain-routing conflation in the legacy winter aggregator**, in which rain water that fell during rain-on-snow events was added to the hourly melt accumulator and broadcast at end-of-day through the snowmelt channel instead of the rain channel. The defect produced a closure hard-stop when an operational replay was audited by the runtime conservation guard; the investigation traced the misrouting to a specific producer line in `winter.for` and a correlated upstream rain-suppression site in `contin.for`. *(legacy defect, surfaced by the runtime guard ten days after the WB-05F release)*

The first three and the fifth had been silently producing wrong-but-plausible numbers in the legacy binary; none was reported as an error because the audit machinery to detect them did not yet exist. Once it did — once the new architecture made it cheap to ask the question — three appeared on the first sweep across our four-project forest corpus, and the rain-routing conflation appeared on a later production-style replay against the runtime conservation guard. The fourth was the first defect surfaced in the new code itself; that it was caught at the gate, on the same residual threshold, is the clearest demonstration that the dual-basis architecture works as designed. All five are repaired and regression-tested; the first four are proven across all 1,166 hillslopes in the forest corpus, and the rain-routing fix is closure-validated on the originally-failing tuples with watchlist verification deferred to a scoped follow-on package.
...
### 6. Rain-routing conflation in legacy winter aggregator (`winter.for` / `contin.for` — `insensible-aliquot/p26` incident)

WEPP separates liquid-water inputs into channels: rain enters through one accounting path, snowmelt enters through another, and the closure audit reconciles both. The legacy daily winter aggregator in `winter.for` was violating that channel separation. Inside the hourly loop that builds the day's total melt (`totmel`), rain that fell during any hour was being **added into the hourly melt accumulator** whenever it coincided with snow on the ground, then broadcast at end-of-day as the day's `wmelt` value. A correlated suppression site in `contin.for` zeroed the rain channel itself on the same days, so the rain water vanished from its proper channel and re-emerged through the melt channel.

For days when the snowpack survived the rainfall, the conflation produced a numerically incorrect but unaudited residue: rain was being credited as melt, with the kernel accepting it because the snowpack-availability cap had headroom. For days when the rainfall **exhausted the snowpack** — the rain-on-snow event that ends with a bare-ground snowpack — the broadcast value at end-of-day was large (the full day's rain), the snowpack at the kernel boundary was zero, and the WBK01 snowpack-availability cap correctly clamped the melt input to zero. The output side, meanwhile, had already been computed against the unclamped value, so the kernel saw zero input and a non-zero output and tripped the conservation guard.

The defect surfaced on an operational replay of the `insensible-aliquot/p26` run on 2026-05-14. The runtime conservation guard tripped on year 1 day 84, OFE 1, with a residual of −3.95 mm. The investigation chain (Track 1 through Track 1D, recorded under `docs/ablation/20260514_insensible-aliquot_p26_hillslope_watbal-process-closure-205-ablation/`) walked the symptom from the kernel-boundary clamp back through the upstream broadcast in `winter.for`, then through the rain-on-snow add at the same routine, and confirmed the misrouting numerically against four independent failing tuples.

The repair (**Candidate 1 rain-routing contract**, recorded in [`contracts/wbk08-clamp-preserve-mass-closure-invariant.md`](contracts/wbk08-clamp-preserve-mass-closure-invariant.md)) is structural rather than tuned: rain water always enters the kernel through the rain channel regardless of snowpack state, and `wmelt` carries only energy-balance snowmelt. The rain-on-snow add in `winter.for` was removed and replaced with a separate liquid-rain return-channel write; the correlated rain-suppression site in `contin.for` was removed so that rain remains in its declared channel throughout the day. Post-fix residuals on the four originally-failing tuples are at floating-point noise (±5 × 10⁻¹⁵ mm). The full 50-year `p26` replay now completes cleanly under the runtime conservation guard with no closure trips.

A separate clamp-event surveillance counter, added during the investigation as Phase 1 instrumentation, additionally revealed twenty-three small events across the 50-year run where the snowpack-availability cap fires on legitimate energy-balance melt that briefly exceeds the producer's per-hour snowpack accounting. These events do not break conservation (the run closes at noise floor on every tuple), but they indicate a separate producer-correctness concern that has been split into its own scoped investigation under [`ablation/20260515_winter_producer_overcomputation/`](ablation/20260515_winter_producer_overcomputation/). That follow-on inherits the Phase 1 counter's failing-tuple manifest as its starting point and is gated on the same closure invariant the rest of the program uses.

Impact: rain-on-snow days in forest runs that exhaust the snowpack mid-day no longer trip the runtime conservation guard. Closure on the originally-failing tuples is at floating-point noise. Production `H.wat`/`H.pass` outputs will read differently on rain-on-snow days where the legacy conflation was misrouting rain into the melt column — rain water now appears in the rain channel where it physically belongs, and the snowmelt channel reports only true energy-balance melt. Aggregate water balance on affected hillslopes tightens; well-conditioned runs without rain-on-snow events are unchanged. Watchlist verification of the behavioral surface area is deferred to the follow-on package.
...
### 2026-05-15 — Rain-routing conflation repaired under Candidate 1 contract
Applied the Candidate 1 rain-routing contract (rain → rain channel, `wmelt` → energy-balance melt only) as the structural fix. Removed the rain-on-snow add in `winter.for:459-461` and the correlated upstream rain-suppression site in `contin.for:856-871`. Added a separate liquid-rain return-channel write so rain remains in its declared channel throughout the day. Post-fix residuals on all four originally-failing tuples are at floating-point noise (±5 × 10⁻¹⁵ mm). The `p26` replay completes cleanly under the runtime guard with no closure trips. Architectural invariant note added at [`docs/contracts/wbk08-clamp-preserve-mass-closure-invariant.md`](contracts/wbk08-clamp-preserve-mass-closure-invariant.md) capturing the clamp-preserve mass-closure invariant, the four review gates (Gate 0 producer correctness, Gate 1 clamp propagation, Gate 2 preserve-path safety, Gate 3 closure-on-normalized-domain), and the Phase 1 → Phase 2 → Phase 3 phasing model for surveillance promotion. The incident package is **resolved under Candidate 1** for the rain-routing defect class; Gate 2 (preserve-path conservation under input suppression) is satisfied for this run. A separate cohort of twenty-three small clamp events surfaced by the Phase 1 counter — distinct from the rain-routing class, indicating a producer-correctness concern where energy-balance melt briefly exceeds per-hour snowpack accounting — has been split into the follow-on package [`ablation/20260515_winter_producer_overcomputation/`](ablation/20260515_winter_producer_overcomputation/) with the counter's failing-tuple manifest as its starting evidence. Watchlist verification of Candidate 1's behavioral surface area is deferred to the follow-on. The current release artifacts (`wepp_260504` and `wepp_260504_hill`) remain the accepted release posture; rebuild and vendor of a rain-routing-corrected binary are operator-controlled release steps not yet executed.
...
The water-balance physics you know is the same physics. It is now organized into independently auditable pieces and instrumented to emit a conservation residual on every step. That instrumentation has already exposed and repaired five real defects: four in the legacy accounting — a transport-capacity bypass that let the bottom OFE discharge runoff above its physical limit, a snowmelt double-count that made every forest hillslope read as non-conserving on melt days, a missing storage export that made interception water disappear from any audit that read `H.wat`, and a rain-routing conflation that aliased rain water into the snowmelt channel on rain-on-snow days — plus one in the new process kernels themselves, surfaced by the runtime conservation guard before any operational run was issued against it. The first four are repaired and proven across the full 1,166-hillslope forest corpus; the rain-routing fix is closure-validated on the originally-failing tuples at floating-point noise, with watchlist verification deferred to a scoped follow-on package. All repairs were structural — not threshold relaxations — and all were caught either by the post-hoc closure audit or by the runtime conservation guard before reaching production output. WB-08A also repaired the cutover-specific hybrid trajectory failure and produced validated `wepp_260504` / `wepp_260504_hill` release artifacts. The legacy binary remains available for historical reproducibility; the rain-routing fix is in source and validated but not yet vendored as a new release artifact, and WB-09 owns operator-controlled vendoring/release promotion.
