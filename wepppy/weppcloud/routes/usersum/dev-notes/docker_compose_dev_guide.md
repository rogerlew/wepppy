...
> To run everything as `www-data:webgroup` set `UID=33` and `GID=<webgroup gid>` (retrieve the latter with `getent group webgroup`). Ensure the group exists on the host; Compose passes numeric ids straight through.
