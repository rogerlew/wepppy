...
- **Transport**: `WCForms.serializeForm(form, { format: "json" })` feeds `WCHttp.postJsonWithSessionToken(url_for_run("build-subcatchments-and-abstract-watershed", { prefix: "/rq-engine/api" }), payload, { form })`. The rq-engine route calls `parse_request_payload`, normalizing booleans (`clip_hillslopes`, `walk_flowpaths`, `mofe_buffer`, `bieger2015_widths`) and floats before mutating the `Watershed` singleton and queuing RQ work.
