...
- **Spatial dataset switching**: the template retains spatial radios supported by selectable datasets in the supplied run catalog, plus the stored current mode. It disables modes unsupported by the selected dataset; the controller enables them when datasets change. This keeps interpolated DAYMET/GRIDMET available without a reload while preserving restricted catalogs. Disabled stored selections remain represented in the initial template markup.

### Map Controller Reference (2025 helper migration)
- **DOM contract**: `map_pure_gl.htm` exposes `data-map-action="go|find-topaz|find-wepp"` on toolbar buttons and keeps status hooks (`#mapstatus`, `#mouseelev`, `#drilldown`, `#sub_legend`, `#sbs_legend`) for helper-driven updates. The inspector tabset sits under `#setloc_form [data-tabset]`—maintain its `role`/ARIA attributes so the controller’s tab manager stays accessible when adding panels.
...
- **Module boundaries**: map logic is now split across `map_gl.js` (controller/runtime orchestration), `map_gl_shared.js` (shared constants and pure helpers), `map_gl_layer_control.js` (base/overlay control rendering + ordering), `map_gl_scale_control.js` (unitizer-aware scalebar overlay), and `map_gl_feature_ui.js` (hover tooltip + feature modal). Keep behavior-preserving changes scoped to the owning module when possible.
...
- **Authenticated overlays**: `addGeoJsonOverlay({ url, loadJson })` accepts an optional JSON loader. Controllers serving overlays from authenticated rq-engine routes pass a loader backed by `WCHttp.requestWithSessionToken`; public overlays omit it and retain the default `WCHttp.getJson` path.
- **Testing**: `controllers_js/__tests__/map_gl.test.js` exercises tabset activation, delegated toolbar actions, event emission, elevation success/error, overlay refresh, and Deck/overlay integration using mocked helpers. Extend this suite whenever the map gains new events or touches additional endpoints.
...
- **Transport**: `WCForms.serializeForm(form, { format: "json" })` feeds `WCHttp.postJsonWithSessionToken(url_for_run("build-subcatchments-and-abstract-watershed", { prefix: "/rq-engine/api" }), payload, { form })`. The rq-engine route calls `parse_request_payload`, normalizing booleans (`clip_hillslopes`, `walk_flowpaths`, `mofe_buffer`, `bieger2015_widths`), floats, and integer `mofe_max_ofes` (clamped to `1..19`) before mutating the `Watershed` singleton and queuing RQ work.
- **Testing**: Jest coverage (`__tests__/subcatchment_delineation.test.js`) locks in build submission, delegated color-map toggles, and error emission; pytest coverage (`tests/weppcloud/routes/test_rq_api_subcatchments.py`) guards RedisPrep/queue integration and batch short-circuit behavior. Run both alongside the standard `wctl run-npm test` / `wctl run-pytest` cadence.
...
- `StatusStream` (in `status_stream.js`) is the shared live-telemetry surface for dashboards and controllers. It renders against `[data-status-panel]`/`[data-status-log]` markup, batches DOM updates, bounds retained messages while preferring lifecycle/error entries, defers hidden-page rendering, manages reconnection/backoff, emits `status:*` custom events, and hydrates stack traces through optional fetchers. RQ job lifecycle remains authoritative through `controlBase` polling; ephemeral stream triggers may prompt an immediate poll but must not override server job state.
- Controlled RQ failures with an `error.message` are rendered as the primary
  ...
  behavior. Controlled guidance renders once in the normal summary area; its
  distance, count, and error ID render as labeled metadata below the message,
  and its empty stacktrace/details panel remains hidden.
...
| Surface | Poll target | Interval | Approx max rate | 429 behavior | Source of truth |
| --- | --- | --- | --- | --- | --- |
| `controlBase` default (shared across `set_rq_job_id` callsites) | `/rq-engine/api/jobstatus/<job_id>` | `2000 ms` | `30 req/min` per active job | Status-first (`HTTP 429`); exponential backoff (`x2`) up to `30000 ms`; reset to base interval after next successful poll | `wepppy/weppcloud/controllers_js/control_base.js` (`DEFAULT_POLL_INTERVAL_MS`) |
| `PathCE` `controlBase` override | `/rq-engine/api/jobstatus/<job_id>` | `2000 ms` | `30 req/min` per active job | Inherits `controlBase` status-first 429 backoff/reset behavior | `wepppy/weppcloud/controllers_js/path_ce.js` (`controller.job_status_poll_interval_ms`) |
| Migration status page (non-controller template, tracked here due rq-engine load) | `/rq-engine/api/jobstatus/<job_id>` | `5000 ms` | `12 req/min` per active page | Status-first (`HTTP 429`) with payload fallback (`error.code=rate_limited` or canonical message); backoff to `30000 ms`; reset after success; keep polling | `wepppy/weppcloud/routes/run_0/templates/run_0/rq-migration-status.htm` |
| `PathCE` status/results loop | `api/path_ce/status` + `api/path_ce/results` | `5000 ms` | `12 req/min` each endpoint | N/A (`path_ce` endpoints, not rq-engine job poll routes) | `wepppy/weppcloud/controllers_js/path_ce.js` (`POLL_INTERVAL_MS`) |
| RQ Job Dashboard | `/rq-engine/api/jobinfo/<job_id>` | Dynamic `1000-5000 ms` | `60-12 req/min` | Status-first (`HTTP 429`) with payload fallback; backoff to `30000 ms`; returns to dynamic baseline after success | `wepppy/weppcloud/routes/rq/job_dashboard/templates/dashboard_pure.htm` |
| DEVAL loading page | `/rq-engine/api/jobstatus/<job_id>` | `800 ms` | `75 req/min` while active | Status-first (`HTTP 429`) with payload fallback; backoff to `30000 ms`; reset after success; 429 does not increment `MAX_POLL_ERRORS` | `wepppy/weppcloud/templates/reports/deval_loading.htm` |
| Runs page delete poll | `/rq-engine/api/jobstatus/<job_id>` | Backoff `2000 -> 8000 ms` | `30 -> 7.5 req/min` per job | Retries until max attempts | `wepppy/weppcloud/templates/user/runs2.html` |
| Batch Runner runstate | `/batch/_/<name>/runstate` | `10000 ms` | `6 req/min` | N/A (`batch` endpoint, not rq-engine job poll routes) | `wepppy/weppcloud/controllers_js/batch_runner.js` |
| Run Sync dashboard status | `/rq-engine/api/run-sync/status` | `10000 ms` | `6 req/min` | N/A (status endpoint not under poll limiter) | `wepppy/weppcloud/controllers_js/run_sync_dashboard.js` |
...
1. Use `setTimeout` loops, not `setInterval`, so each cycle schedules exactly one next poll.
2. Keep one timer handle per poller; clear it before scheduling and set it to `null` when firing.
...
`controllers-gl.js` file.
- `control_base.js` now uses `WCHttp`/`WCDom` internally. Controllers may continue supplying legacy jQuery objects, but new code should pass native elements (the base normalizes both).
...
- Long-running work posts `FormData` via `WCHttp.requestWithSessionToken(url_for_run("run-ash", { prefix: "/rq-engine/api" }))` so raster uploads survive, while lightweight toggles (`set_ash_wind_transport`) send JSON through `WCHttp.postJson`. The rq-engine route uses `parse_request_payload` and `Ash.parse_inputs` accepts native booleans/ints/floats.
...
- Audit templates for inline `$()` usage during migrations; move bootstrap logic to dedicated modules so controllers can initialize via `DOMContentLoaded` or direct module execution.
...
- Landuse mapping failures must not block report hydration: when `modify_landuse_mapping_rq` reaches a terminal error (`job:error`) for the active mapping job, the controller still refreshes `report/landuse/` so users can keep working without rebuilding landuse.
- Landuse mode contract for MOFE projects: `Single landuse for watershed` is disabled because MOFE management assignment requires a gridded landuse raster for per-OFE intersections. The Pure template surfaces this in the control description, and rq-engine rejects `mode=1` or build requests that still have persisted Single mode instead of enqueueing a job that would fail later on a missing raster.
- The WEPP migration introduced a scoped `WCEvents` emitter (`wepp:run:*`, `wepp:report:loaded`) and expanded `__tests__/wepp.test.js` to exercise lifecycle signals, delegated uploads, and error propagation. Use this suite when adding new run-control behaviors or subscribing to WEPP events from neighboring modules.
...
- **Transport**: scenario submissions build a `FormData` payload containing a JSON `scenarios` list plus any SBS uploads, then call `WCHttp.requestWithSessionToken(url_for_run("run-omni", { prefix: "/rq-engine/api" }), { method: "POST", body: formData })`. The rq-engine endpoints (`/rq-engine/api/.../run-omni`, `/rq-engine/api/.../run-omni-contrasts`) call `parse_request_payload` so JSON and multipart payloads share a single parser; SBS files stage under `omni/_limbo/{idx}` before Omni clones scenarios.
...
- **Transport**: submissions call `WCHttp.requestWithSessionToken(url_for_run("set-outlet", { prefix: "/rq-engine/api" }), { json: { latitude, longitude }, form })` (or a `{ coordinates: { lat, lng } }` envelope). The rq-engine route consumes payloads via `parse_request_payload`, normalizes floats, clears `RedisPrep` timestamps, and enqueues `set_outlet_rq`.
...
- **Transport**: uploads send `FormData` via `WCHttp.requestWithSessionToken(url_for_run("tasks/upload-sbs/", { prefix: "/rq-engine/api" }))`; removals hit `tasks/remove_sbs`; uniform rasters call `tasks/build_uniform_sbs/<value>`. JSON mutations (`tasks/set_firedate/`, `tasks/modify_burn_class`, `tasks/modify_color_map`) now flow through `parse_request_payload`, which delivers trimmed strings plus native integers for class breaks before reaching the NoDb mods. Map previews read from `query/baer_wgs_map/` and render the SBS legend client-side.
...
- **DOM contract**: `controls/disturbed_sbs_pure.htm` handles SBS upload/uniform actions via `data-sbs-action` + `data-sbs-uniform`; `controls/disturbed_modal.htm` handles lookup lifecycle/actions via `[data-disturbed-action]`, lookup selection radios (`[data-disturbed-lookup-variant]`), and optional selected-link highlighting (`[data-disturbed-modify-link]`). Loading the extended lookup now prompts for explicit confirmation from the load action before the task request is sent. Hint nodes (`#hint_upload_sbs`, `#hint_remove_sbs`, `#hint_low_sbs`, `#hint_moderate_sbs`, `#hint_high_sbs`) remain the inline feedback surface for SBS actions.
- **Event surface**: `Disturbed.getInstance().events = WCEvents.useEventMap(['disturbed:mode:changed', 'disturbed:sbs:state', 'disturbed:lookup:reset', 'disturbed:lookup:extended', 'disturbed:lookup:deleted', 'disturbed:lookup:synced', 'disturbed:lookup:variant', 'disturbed:lookup:error', 'disturbed:upload:started', 'disturbed:upload:completed', 'disturbed:upload:error', 'disturbed:remove:started', 'disturbed:remove:completed', 'disturbed:remove:error', 'disturbed:uniform:started', 'disturbed:uniform:completed', 'disturbed:uniform:error', 'disturbed:firedate:updated', 'disturbed:firedate:error'])`. The controller also dispatches `CustomEvent('disturbed:has_sbs_changed')` for legacy listeners and propagates `controlBase` lifecycle events with task identifiers such as `disturbed:upload` and `disturbed:lookup:reset`.
- **Transport**: lookup lifecycle tasks `POST` empty payloads (`tasks/reset_disturbed`, `tasks/load_extended_land_soil_lookup`, `tasks/delete_extended_land_soil_lookup`, `tasks/sync_base_to_extended_land_soil_lookup`); lookup radio changes persist server-side via `POST tasks/set_lookup_variant` (`{"lookup_variant":"base|extended"}`), and refresh state from `GET api/disturbed/lookup_meta` (including `has_extended_lookup` for extended-control enablement). Explicit `lookup=extended` requests hard-fail with `LOOKUP_VARIANT_UNAVAILABLE` when extended lookup is absent (no silent base fallback). Uniform rasters post JSON to `tasks/build_uniform_sbs` (`{"value": int}`) while retaining the legacy `<value>` path; fire dates post JSON to `tasks/set_firedate/`; uploads remain `FormData`; `has_sbs` polls `api/disturbed/has_sbs/`. Every route runs through `parse_request_payload`, delivering native booleans/ints to the NoDb layer.
...
- **Event surface**: `DebrisFlow.getInstance().events = WCEvents.useEventMap(['debris:run:started', 'debris:run:completed', 'debris:run:error'])`. Pair these with `controlBase` lifecycle broadcasts (`job:started|completed|error`) and the existing `DEBRIS_FLOW_RUN_TASK_COMPLETED` WebSocket trigger to keep telemetry dashboards and neighboring controllers in sync.
- **Transport**: runs post JSON via `WCHttp.postJsonWithSessionToken(url_for_run("run-debris-flow", { prefix: "/rq-engine/api" }), {})`. The rq-engine route relies on `parse_request_payload`, coercing optional `clay_pct` and `liquid_limit` floats plus an optional `datasource` string before enqueuing `run_debris_flow_rq(runid, payload=…)`. The RQ task accepts those native values and forwards them to `DebrisFlow.run_debris_flow(cc=…, ll=…, req_datasource=…)`, eliminating the last `"on"` checks.
...
### Roads Controller Reference (2026 helper migration)
- **DOM contract**: `roads_pure.htm` renders a helper-first upload + queue workflow under `#roads_form`: file input `#roads_geojson_file`, progress shell `#roads_geojson_file-progress`, upload action `data-roads-action="upload"`, queue actions `data-roads-action="prepare-segments|run"`, status `#roads_status_panel`, stacktrace `#roads_stacktrace_panel`, and job hint `#hint_run_roads`. Keep these IDs/hooks stable so dynamic mod toggles and status telemetry continue to work.
- **Event surface**: `Roads.getInstance().events = WCEvents.useEventMap(['roads:upload:started', 'roads:upload:completed', 'roads:upload:error', 'roads:prepare:started', 'roads:prepare:completed', 'roads:prepare:error', 'roads:run:started', 'roads:run:completed', 'roads:run:error', 'job:started', 'job:completed', 'job:error'])`. Queue completion must remain idempotent for both websocket triggers and poll fallback (`ROADS_PREPARE_TASK_COMPLETED`, `ROADS_RUN_TASK_COMPLETED`).
- **Transport**: upload posts multipart payloads to `url_for_run("tasks/roads/upload_geojson")` with CSRF header support from `WCHttp.getCsrfToken(form)`. Prepare/run posts use `WCHttp.request(url_for_run("tasks/roads/prepare_segments|run"), { method: "POST", form })`, then set `poll_completion_event` before `set_rq_job_id` so `controlBase` poll fallback mirrors status-stream triggers.
- **Testing**: controller behavior is covered by `controllers_js/__tests__/roads.test.js` (upload progress, queue submissions, completion idempotency). Template contract is covered by `tests/weppcloud/routes/test_pure_controls_render.py` (`controls/roads_pure.htm` assertions). Run `wctl run-npm test -- roads`, rebuild with `python wepppy/weppcloud/controllers_js/build_controllers_js.py`, and execute targeted pytest for route/template checks when touching this domain.

### Geneva Controller Reference (2026 helper migration)
- **DOM contract**: `geneva_pure.htm` renders a helper-first control shell under `#geneva_form` and seeds endpoint/config data through `#geneva_controller_data[data-geneva-config]`. Geneva queue submission is a single delegated action `data-geneva-action="run-workflow"` on `#geneva_run_batch`; the summary, status, stacktrace, and results nodes remain in the standard control shell (`#info`, `#status`, `#stacktrace`, `#geneva-results`). Queue job hints now render below the submit control via `#hint_run_geneva_run_workflow` (with legacy hint ids retained as hidden fallbacks).
- **Event surface**: `Geneva.getInstance().events = WCEvents.useEventMap(['geneva:state:loaded', 'geneva:config:saved', 'geneva:workflow:queued', 'geneva:state:error'])`. Queue completion reconciles through the rq-engine state endpoint, and `GENEVA_STATE_SYNC_REQUESTED` is the controller's completion trigger for websocket or poll fallback refresh.
- **Transport**: configuration still posts JSON to Flask `api/geneva/config`, but runtime state and queue submission use rq-engine session-token flows: `GET /rq-engine/api/.../geneva/state` and `POST /rq-engine/api/.../geneva/run-workflow`. The workflow payload nests `prepare`, `panel`, and `run_batch`; rq-engine enforces `prepare.force_rebuild=true` and `panel.rebuild=true` before enqueuing the chain (`prepare -> panel -> run_batch`). Bootstrap `jobIds` remain last-known hints only; active job locks and config hydration must come from the rq-engine Geneva state payload.
- **Testing**: front-end coverage lives in `controllers_js/__tests__/geneva.test.js`, template/render coverage in `tests/weppcloud/routes/test_pure_controls_render.py`, rq-engine coverage in `tests/microservices/test_rq_engine_geneva_routes.py`, and bootstrap wiring coverage in `tests/weppcloud/routes/test_run_0_openet_admin_gate.py`. Run `wctl run-npm lint`, `wctl run-npm test -- geneva`, rebuild the bundle, and execute the targeted pytest slices whenever Geneva controller or state contracts change.

### Geneva Summary Report Controller Reference (2026 map update)
- **Map layer stack**: `geneva_summary_report.js` renders a Google Terrain deck.gl `TileLayer`/`BitmapLayer` as the base layer (`geneva-summary-base-google-terrain`) and overlays the HRU choropleth `GeoJsonLayer` above it.
- **Behavior contract**: map clear/reset paths must preserve the terrain base layer so empty/error states still render geospatial context while HRU overlays are removed.
- **Selection/layout contract**: map event source is `selected_storm_id` only (chart marker and table row selection); the report no longer exposes a dedicated map-event dropdown. Template `reports/geneva/summary.htm` overlays the legend inside the map viewport (top-left), keeps `geneva-summary__map-layout` single-column/full-width at desktop breakpoints, and renders map empty/error alerts below the map status line.
- **Interaction contract**: wheel zoom is cooperative (requires `Ctrl` + scroll); non-`Ctrl` wheel input is allowed to bubble to page scroll and is blocked from map zoom via capture-phase wheel handling on the map canvas. Report UI includes an on-map tip and deck tooltip styling for readability.
- **Transport**: base tiles resolve from `https://{s}.google.com/vt/lyrs=p&x={x}&y={y}&z={z}` (`mt0..mt3` subdomains); HRU map geometry and rows continue to load from run-scoped Geneva query endpoints.
- **Testing**: `controllers_js/__tests__/geneva_summary_report.test.js` validates that map renders include both base and overlay layers and that immutable `fitBounds()` return values are handled safely.
...
- **Transport**: submissions post JSON via `WCHttp.postJsonWithSessionToken(url_for_run("post-dss-export-rq", { prefix: "/rq-engine/api" }), { dss_export_mode, dss_export_channel_ids, dss_export_exclude_orders })` so the session JWT is minted on demand. The rq-engine route mirrors Flask’s `parse_request_payload`, coalescing JSON or legacy form posts into native ints/booleans, deriving channel IDs from `Watershed.chns_summary` when mode 2 is requested, and persisting via NoDb property setters (`wepp.dss_export_mode`, `wepp.dss_excluded_channel_orders`, `wepp.dss_export_channel_ids`).
...
- **Transport**: Selection summaries post JSON to `query/rangeland_cover/current_cover_summary/` (returning the canonical cover mix). Box selections stream extents to `tasks/sub_intersection/`. Submit calls `tasks/modify_rangeland_cover/` with `{ topaz_ids, covers }`, where the controller already coerces covers to native floats and deduplicates Topaz IDs. The Flask route now uses `parse_request_payload`, normalizes ID arrays, validates that every cover falls within `0–100`, and returns `exception_factory` payloads when validation fails or an unknown Topaz ID is supplied.
...
- **Transport**: Mode changes post JSON to `tasks/set_treatments_mode/` with `{ mode, single_selection }`. The Flask route now uses `parse_request_payload`, accepts JSON or legacy form posts, coerces `mode` to the `TreatmentsMode` enum, and continues returning `success_factory()` for compatibility. Build submissions stream `FormData` (plus optional rasters) via `WCHttp.requestWithSessionToken(url_for_run("build-treatments", { prefix: "/rq-engine/api" }), { method: "POST", body: formData })`, with the controller emitting run lifecycle events and forwarding StatusStream job telemetry.
...
- **Transport**: Job submissions call `WCHttp.postJsonWithSessionToken(url_for_run("run-rhem", { prefix: "/rq-engine/api" }), WCForms.serializeForm(form, { format: "object" }))`. The rq-engine route normalizes JSON or form posts via `parse_request_payload`, coercing the `clean`, `prep`, and `run` flags before enqueuing `run_rhem_rq(payload=…)`. The RQ task honors those booleans, skipping `clean()`, `prep_hillslopes()`, or `run_hillslopes()` when a flag is `False` while still publishing StatusStream telemetry.
...
- **Transport**: Invites post JSON to `tasks/adduser/` with `{ email }` (legacy `adduser-email` form payloads remain accepted). Removals post `{ user_id }` to `tasks/removeuser/`. The project blueprint normalizes inputs with `parse_request_payload`, consults `user_datastore.find_user`, handles duplicate membership idempotently (`Content.already_member` / `Content.already_removed`), and preserves historical logging/exception semantics through `success_factory` / `error_factory`.
...
- **Event surface**: `RAP_TS.getInstance().events = WCEvents.useEventMap(['rap:schedule:loaded', 'rap:timeseries:run:started', 'rap:timeseries:run:completed', 'rap:timeseries:run:error', 'rap:timeseries:status', 'job:started', 'job:completed', 'job:error'])`. Subscribe to these helper-first emissions—or the inherited `job:*` lifecycle events—in dashboards and neighboring modules instead of scraping DOM text or listening for the legacy `RAP_TS_TASK_COMPLETED` trigger.
- **Transport**: Job submissions call `WCHttp.postJsonWithSessionToken(url_for_run("acquire-rap-ts", { prefix: "/rq-engine/api" }), WCForms.serializeForm(form, { format: "json" }))`. The rq-engine route normalizes optional `datasets`, `schedule`, and `force_refresh` fields via `parse_request_payload` before enqueuing `fetch_and_analyze_rap_ts_rq(payload=…)`, which logs the metadata then runs the climate-driven acquisition pipeline.
...
### Path CE Controller Reference (PATH-CE v2)
- **DOM contract**: `path_cost_effective_pure.htm` exposes helper-friendly hooks: thresholds and filters ship with `name` attributes so `WCForms.serializeForm` can hydrate, treatment rows render through a delegated `<tbody>` driven by `data-pathce-field` controls (readonly derived `label` and `quantity`, a `scenario` `<select>` over the mulch scenarios, `unit_cost`, `fixed_cost` — label and rate re-derive on scenario change), and user actions run through buttons tagged `data-pathce-action="run" | "add-treatment" | "remove-treatment"` (no save button: the run payload persists the config server-side). New v2 surfaces: `#path_ce_preconditions` (actionable Omni-provisioning failures parsed from status messages) and `#path_ce_results_panel` (a WEPP-Run-Results-style panel rendered into `#path_ce_links`: report + artifact CSV links, each with a description hint; hidden until results exist). Status, hint, stacktrace, and job metadata live under the `#path_ce_form` shell so `controlBase` manages telemetry without inline handlers.
- **Units**: the Sddc threshold input and treatment `unit_cost` inputs are unitizer-managed (`weight-annual` canonical `tonne/yr`; `currency-area` canonical `$/acre` per ADR-0023/D4); the treatments table's Unit Cost header carries a live `data-unitizer-label` span. The controller stores canonical values in `dataset.unitizerCanonicalValue` and prefers them at harvest only when `UnitizerClient` is present; the Sdyd threshold stays a plain input (its artifact unit, tonnes/acre, has no unitizer category). Summary Sddc values (Total Sddc Reduction, Final Sddc) render through `UnitizerClient.renderValue` multi-unit blocks (canonical `tonne/yr` text as the no-client fallback), so they follow the global unit preference like report tables do.
- **Event surface**: `PathCE.getInstance().events = WCEvents.useEventMap(['pathce:config:loaded', 'pathce:config:error', 'pathce:treatment:added', 'pathce:treatment:removed', 'pathce:treatment:updated', 'pathce:status:update', 'pathce:results:update', 'pathce:run:started', 'pathce:run:completed', 'pathce:run:error', 'job:started', 'job:completed', 'job:error'])`.
- **Transport**: Configuration loads via `WCHttp.getJson("api/path_ce/config")`. Run submissions call `WCHttp.postJson("tasks/path_cost_effective_run", {sddc_threshold, sdyd_threshold, slope_range, severity_filter, treatments[]})` — the route merges the payload into the persisted config before enqueueing, so Run is also the save path (the pre-v2 `mulch_costs`/`treatment_options` shape is gone; the blueprint passes values through for the NoDb controller's strict normalization, so present-but-invalid fields return errors rather than coercing). Runs are rejected while a PATH-CE job is already queued/running; the HTML report always renders. Results carry `report.html` (served inline by the browse microservice at `report/path_ce/` with a sandbox CSP) plus artifact relpaths for `download/<relpath>?as_csv=1` links.
- **Testing**: Jest coverage in `controllers_js/__tests__/path_ce.test.js` exercises config hydration, treatment table interactions, payload contract, report/download link rendering, precondition messaging, and lifecycle events. Backend assertions live in `tests/weppcloud/routes/test_path_ce_bp.py` and `tests/microservices/test_browse_report_routes.py`. Run `wctl run-npm test -- path_ce`, rebuild the bundle (`python wepppy/weppcloud/controllers_js/build_controllers_js.py`), and execute `wctl run-pytest tests/weppcloud/routes/test_path_ce_bp.py tests/nodb/mods/path_ce` when iterating on the PATH Cost-Effective domain.

### Batch Runner Controller Reference (2025 helper migration)
- **DOM contract**: The Pure template (`batch_runner_pure.htm`) exposes delegated hooks via `data-action="batch-upload"`, `data-action="batch-validate"`, and `data-action="batch-run"`. Run-directive checkboxes render with `data-run-directive="<slug>"`, while the resource and validation panels rely on the existing `data-role="upload-status"`, `data-role="resource-*"` (meta, schema, samples) and `data-role="validation-*"` nodes. Status, stacktrace, job hint, and the batch-progress summary live under `#batch_runner_form` so `controlBase` plus the controller manage telemetry without inline handlers.
...
- **Transport**: uploads stream `FormData` to `/rq-engine/api/batch/_/<name>/upload-geojson` (and `/rq-engine/api/batch/_/<name>/upload-sbs-map`); template checks post JSON to `/batch/_/<name>/validate-template`; directive toggles post JSON to `/batch/_/<name>/run-directives`; batch submissions post JSON to `/rq-engine/api/batch/_/<name>/run-batch`. Flask routes now consume payloads via `parse_request_payload`, and `BatchRunner.update_run_directives` coerces stringy truths (`"true"`, `"false"`, `"on"`, `"off"`) to native booleans so NoDb state stays clean. Batch progress polling reads `/batch/_/<name>/runstate`, rendering the CLI-style runstate report in a responsive 5-column grid and refreshing every 10s (LPT ordering note is displayed inline).
- **Testing**: Jest coverage in `controllers_js/__tests__/batch_runner.test.js` exercises upload/validate flows, directive persistence, run submission, event emission, and runstate polling. Backend expectations live in `tests/weppcloud/test_batch_runner_endpoints.py` (upload, validation, directives) and `tests/weppcloud/routes/test_rq_api_batch_runner.py` (queue wiring and error handling). Run `wctl run-npm test -- batch_runner`, rebuild the bundle (`python wepppy/weppcloud/controllers_js/build_controllers_js.py`), and execute `wctl run-pytest tests/weppcloud/test_batch_runner_endpoints.py tests/weppcloud/routes/test_rq_api_batch_runner.py` before handing off changes.

### AgFields Controller Reference (2026 staged workflow)
- **DOM contract**: `controls/ag_fields_pure.htm` renders four numbered panels under `#ag_fields_form`; controller-addressable elements use the `data-role` hooks specified in `wepppy/nodb/mods/ag_fields/ui_control_layout.md`. Stage 1 hydrates `boundary.filename` into a dedicated current-file display because browsers cannot restore file-input values after reload. The crop mapping dialog is `#agfields_rotation_modal`, and all actions are delegated through `data-action` attributes so dynamic mod insertion remains safe.
...
- `rq-engine/api/*` — Background job triggers (build-climate, build-landuse, build-soils, build-treatments, run-wepp, run-wepp-watershed, run-omni, run-ash, run-debris-flow, run-rhem, acquire-rap-ts, acquire-openet-ts, post-dss-export-rq, fetch-dem-and-build-channels, etc.)
- `rq/api/*` — Removed; use `/rq-engine/api` for background job triggers and polling
...
**Upload endpoints:** RQ-engine run endpoints accept multipart payloads directly under `/rq-engine/api` with session tokens; `/upload` is deprecated/removed.

```javascript
http.requestWithSessionToken(url_for_run("tasks/upload-sbs/", { prefix: "/rq-engine/api" }), { method: "POST", body: formData })
...
pattern = r'(?<!url_for_run\()"(tasks/|query/|resources/)([^"]+)"'
