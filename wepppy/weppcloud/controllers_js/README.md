...
- **DOM contract**: `geneva_pure.htm` renders a helper-first control shell under `#geneva_form` and seeds endpoint/config data through `#geneva_controller_data[data-geneva-config]`. Queue buttons publish delegated clicks via `data-geneva-action="save-config|refresh-state|prepare|build-panel|run-batch"`, while the summary, status, stacktrace, hint, and results nodes remain in the standard control shell (`#info`, `#status`, `#stacktrace`, `#hint_run_geneva`, `#geneva-results`) so `controlBase` can manage telemetry.
- **Event surface**: `Geneva.getInstance().events = WCEvents.useEventMap(['geneva:state:loaded', 'geneva:config:saved', 'geneva:prepare:queued', 'geneva:panel:queued', 'geneva:run:queued', 'geneva:state:error'])`. Queue completion reconciles through the rq-engine state endpoint, and `GENEVA_STATE_SYNC_REQUESTED` is the controller's completion trigger for websocket or poll fallback refresh.
- **Transport**: configuration still posts JSON to Flask `api/geneva/config`, but runtime state and queue actions use rq-engine session-token flows: `GET /rq-engine/api/.../geneva/state`, `POST /rq-engine/api/.../geneva/prepare-hrus`, `POST /rq-engine/api/.../geneva/build-frequency-panel`, and `POST /rq-engine/api/.../geneva/run-batch`. Bootstrap `jobIds` remain last-known hints only; active job locks and config hydration must come from the rq-engine Geneva state payload.
