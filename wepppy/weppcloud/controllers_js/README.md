...
- **DOM contract**: templates expose `data-omni-action` hooks for primary buttons (`add-scenario`, `run-scenarios`) plus delegated selectors tagged with `data-omni-role="scenario-select"` and panels marked with `data-omni-scenario-controls`. The controller renders scenario rows at runtime—no inline scripts remain—so keep markup minimal and document any new `data-omni-*` attributes whenever scenarios or controls expand.
