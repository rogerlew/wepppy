...
var SubcatchmentDelineation = (function () {
    ...
    function renderLegend(name, data, emit) {
        if (!name) {
            return Promise.resolve();
        }
        var html = "";
        if (name === "landuse") {
            html = buildLegendHtml("Landuse Legend", buildLegendItems(data && data.landuse, { keyField: "key" }));
        } else if (name === "soils") {
            html = buildLegendHtml("Soils Legend", buildLegendItems(data && data.soils, { keyField: "mukey" }));
        } else {
            return Promise.resolve();
        }
        setSubLegend(html);
    ...
    function createInstance() {
        ...
        function renderLayer(options) {
            ...
            return requestJson(targetUrl)
                ...
                .then(function () {
                    if (legend) {
                        return renderLegend(legend, { landuse: state.dataLanduse, soils: state.dataSoils }, emit);
                    }
                    return undefined;
                });
        }
