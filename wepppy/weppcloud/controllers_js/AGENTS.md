...
- **Contract authority**: follow
  `docs/standards/contract-first-change-standard.md`. Applicable current
  canonical domain and shared/cross-
  cutting contracts are normative; controller, template, route, and test
  behavior is conformance evidence. An intended change requires the accepted
  contract-decision checkpoint and ancestor revision before controller edits.
  Amend every applicable contract before changing intended payloads, events, fields,
  defaults, or lifecycle behavior. Never infer a replacement contract from
  current code or an archived plan; archived plans are read-only historical
  leads.
- **Documentation alignment**: after the contract-first amendment, update
  `controllers_js/README.md` and this playbook when shared guidance changes.
...
5. **Document**: update `README.md` and this playbook when shared guidance changes; the domain contract must already reflect the intended behavior.
6. **Validate**:
...
- When controllers need event busses, prefer `WCEvents.createEmitter` or `WCEvents.emitDom`; `wepp.js` now emits `wepp:run:*` and `wepp:report:loaded` so neighboring modules subscribe without polling DOM state.
...
- Jest coverage for the controller lives in `__tests__/project.test.js`. When events, payloads, or data hooks change, expand that suite alongside template updates to lock in behavior.
...
- `Climate.getInstance().events` wraps `WCEvents.useEventMap` and currently publishes `climate:dataset:changed`, `climate:dataset:mode`, `climate:station:mode`, `climate:station:selected`, `climate:station:list:loading`, `climate:station:list:loaded`, `climate:build:started`, `climate:build:completed`, `climate:build:failed`, `climate:precip:mode`, `climate:upload:completed`, `climate:upload:failed`, `climate:gridmet:updated`, `climate:mxpt5:updated`, and `climate:silent-pass-observed-quality-guard:updated`. Use these signals for dashboards and downstream automation.
- Network flows run through `WCHttp.postJsonWithSessionToken`/`request`; rq-engine now handles `/rq-engine/api/.../build-climate` via `parse_request_payload` so JSON and form submissions behave the same. Update `wepppy/weppcloud/routes/nodb_api/climate_bp.py` alongside controller changes.
...
- **Map controller**: `map_pure_gl.htm` uses `data-map-action="go|find-topaz|find-wepp"` on toolbar buttons while keeping `#mapstatus`, `#mouseelev`, `#drilldown`, `#sub_legend`, and `#sbs_legend` for helper-based updates. `MapController.getInstance().events = WCEvents.useEventMap([...])` emits `map:ready`, `map:center:requested`, `map:center:changed`, `map:search:requested`, `map:elevation:requested`, `map:elevation:loaded`, `map:elevation:error`, `map:drilldown:requested`, `map:drilldown:loaded`, `map:drilldown:error`, `map:layer:toggled`, `map:layer:refreshed`, and `map:layer:error`. Subscribe to those signals (instead of scraping DOM) whenever neighboring controls react to viewport, drilldown, or overlay state.
- **Map helper split**: keep shared constants/pure utilities in `map_gl_shared.js`, layer-control rendering/ordering in `map_gl_layer_control.js`, scale overlay logic in `map_gl_scale_control.js`, and hover/modal presentation in `map_gl_feature_ui.js`; reserve `map_gl.js` for controller orchestration and public API wiring.
...
- Build requests post JSON with `WCHttp.postJsonWithSessionToken(url_for_run("build-subcatchments-and-abstract-watershed", { prefix: "/rq-engine/api" }), WCForms.serializeForm(form, { format: "json" }))`. The rq-engine route runs through `parse_request_payload`, converting booleans (`clip_hillslopes`, `walk_flowpaths`, `mofe_buffer`, `bieger2015_widths`), floats, and integer `mofe_max_ofes` (clamped to `1..19`) before mutating the `Watershed` singleton and queuing RQ work.
...
- Transport: modification submissions post JSON to `url_for_run("modify-landuse", { prefix: "/rq-engine/api" })` (`{ topaz_ids: [...], landuse: '<code>' }`) via `WCHttp.requestWithSessionToken`. Rectangle selections call `tasks/sub_intersection/` with a JSON extent. Record schema changes in the current canonical domain contract.
- Testing: Jest coverage (`controllers_js/__tests__/landuse_modify.test.js`) exercises selection toggles, event emission, and error handling with stubbed MapController/Leaflet helpers. Backend behavior is locked down in `tests/weppcloud/routes/test_landuse_bp.py`—run those alongside `wctl run-npm test` and `wctl run-pytest tests/weppcloud/routes/test_landuse_bp.py tests/weppcloud/routes/test_rq_api_landuse.py` during refactors.

### Omni controller reference
- DOM hooks now rely on `data-omni-action` (`add-scenario`, `run-scenarios`, `delete-selected`, `confirm-delete`), delegated scenario selectors tagged with `data-omni-role="scenario-select"`, opt-in deletion checkboxes tagged `data-omni-role="scenario-select-toggle"`, and per-row containers marked `data-omni-scenario-controls`. Templates no longer ship inline scripts—let the controller render scenario cards dynamically and document any new `data-omni-*` hooks when requirements change.
...
- Scenario submissions build a `FormData` payload containing `scenarios` JSON plus optional SBS uploads and call `WCHttp.requestWithSessionToken(url_for_run("run-omni", { prefix: "/rq-engine/api" }))`; rq-engine routes `/rq-engine/api/.../run-omni` and `/rq-engine/api/.../run-omni-contrasts` call `parse_request_payload` so JSON and multipart submissions share one parser. SBS files stage in `omni/_limbo/{idx}` before payloads hydrate `Omni.parse_scenarios` with native values (no `"on"` strings).
...
- Submissions call `WCHttp.requestWithSessionToken(url_for_run("set-outlet", { prefix: "/rq-engine/api" }), { json: { latitude, longitude }, form })`; the rq-engine route consumes JSON/form payloads through `parse_request_payload`, normalizes floats, clears `RedisPrep` timestamps, and enqueues `set_outlet_rq`.
...
- Transport: uploads submit `FormData` via `WCHttp.requestWithSessionToken(url_for_run("tasks/upload-sbs/", { prefix: "/rq-engine/api" }))`; removals hit `tasks/remove_sbs`; uniform maps call `tasks/build_uniform_sbs/<value>`. JSON endpoints (`tasks/set_firedate/`, `tasks/modify_burn_class`, `tasks/modify_color_map`) flow through `parse_request_payload` so class breaks arrive as ints and nodata tokens stay trimmed. Map previews request `query/baer_wgs_map/` and render the SBS legend client-side.
- Tests: update `controllers_js/__tests__/baer.test.js` for UI/HTTP behavior and `tests/weppcloud/routes/test_disturbed_bp.py` for backend parsing. Always run `wctl run-npm lint`, `wctl run-npm test -- baer` (or the full suite), rebuild the bundle, and execute the disturbed blueprint pytest cases when BAER changes land.
- Documentation: capture new payload fields, emitted events, and helper expectations in the current canonical domain contract. Archived controller plans are read-only historical leads.

### Disturbed controller reference
- DOM hooks: Pure SBS markup now depends on `data-sbs-action`, `data-sbs-uniform`, and `[data-disturbed-action]` plus the legacy hint nodes (`#hint_upload_sbs`, `#hint_remove_sbs`, `#hint_low_sbs`, `#hint_moderate_sbs`, `#hint_high_sbs`). Disturbed modal lookup controls use `[data-disturbed-lookup-variant]` and optional `[data-disturbed-modify-link]` selectors so the controller can keep base vs extended state visible. Keep new affordances helper-friendly—no inline handlers.
...
- Transport: lookup lifecycle actions issue `POST` requests with empty bodies (`tasks/reset_disturbed`, `tasks/load_extended_land_soil_lookup`, `tasks/delete_extended_land_soil_lookup`, `tasks/sync_base_to_extended_land_soil_lookup`) and selection status is refreshed from `api/disturbed/lookup_meta`; uniform builds post JSON to `tasks/build_uniform_sbs` (`{"value": int}`) while the `<value>` route remains for older clients; fire dates post JSON to `tasks/set_firedate/`; uploads use `WCHttp.requestWithSessionToken(url_for_run("tasks/upload-sbs/", { prefix: "/rq-engine/api" }))` with `FormData`; `has_sbs` polls `api/disturbed/has_sbs/`. All routes parse through `parse_request_payload` so downstream code receives native types.
...
- Events: `DebrisFlow.getInstance().events = WCEvents.useEventMap(['debris:run:started', 'debris:run:completed', 'debris:run:error'])`. Emit these alongside the `controlBase` lifecycle events (`job:started|completed|error`) whenever runs queue, finish, or fail so dashboards and neighbors stay aligned.
- Transport: submissions call `WCHttp.postJsonWithSessionToken(url_for_run("run-debris-flow", { prefix: "/rq-engine/api" }), payload)`; the rq-engine route runs the body through `parse_request_payload`, coercing optional `clay_pct` / `liquid_limit` floats and an optional `datasource` string before enqueuing `run_debris_flow_rq(runid, payload=…)`. The RQ task forwards the native values to `DebrisFlow.run_debris_flow(cc=…, ll=…, req_datasource=…)`—no more `"on"` checks.
...
- Documentation: record payload schemas, event contracts, and follow-up tasks in the current canonical domain contract. Treat the archived Debris Flow plan as read-only history.
...
- Tests: Jest coverage in `controllers_js/__tests__/rangeland_cover_modify.test.js` checks summary hydration, payload composition, event emission, and validation errors. Backend expectations sit in `tests/weppcloud/routes/test_rangeland_cover_bp.py`, confirming Topaz normalization, range checks, and legacy fallbacks. Run `wctl run-npm test -- rangeland_cover_modify`, rebuild the bundle, execute the targeted pytest, and amend the current canonical domain contract before intended behavior changes.
...
- Transport: mode changes call `WCHttp.postJson("tasks/set_treatments_mode/", { mode, single_selection })`. The Flask blueprint now funnels requests through `parse_request_payload`, accepts JSON or legacy form bodies, coerces `mode` to the `TreatmentsMode` enum, and still responds with `success_factory()`. Build submissions stream `FormData` (optional rasters included) via `WCHttp.requestWithSessionToken(url_for_run("build-treatments", { prefix: "/rq-engine/api" }), { method: "POST", body: formData })` so session JWTs are minted on demand.
...
- Transport: submissions call `WCHttp.postJsonWithSessionToken(url_for_run("run-rhem", { prefix: "/rq-engine/api" }), WCForms.serializeForm(form, { format: "object" }))`. The rq-engine route runs the body through `parse_request_payload`, coercing the `clean`, `prep`, and `run` flags before enqueuing `run_rhem_rq(payload=…)`. The RQ task honors those booleans—skipping `clean()`, `prep_hillslopes()`, or `run_hillslopes()` when a flag is `False`—while still publishing StatusStream telemetry.
...
- Telemetry: the controller attaches to `StatusStream` via `controlBase.attach_status_stream`; `rhem.appendStatus` fans updates to both the DOM and event bus so dashboards stay synchronized.
...
- Transport: invites call `WCHttp.postJson("tasks/adduser/", { email })` (legacy `adduser-email` form payloads still work), removals call `WCHttp.postJson("tasks/removeuser/", { user_id })`. The Flask blueprint runs payloads through `parse_request_payload`, leans on `user_datastore.find_user`, and returns `{ Content: { already_member|already_removed } }` for idempotent operations.
...
- Docs: record behavior and payload changes in the current canonical domain contract; the archived Team plan is read-only history.
...
- Events: `RAP_TS.getInstance().events = WCEvents.useEventMap(['rap:schedule:loaded', 'rap:timeseries:run:started', 'rap:timeseries:run:completed', 'rap:timeseries:run:error', 'rap:timeseries:status', 'job:started', 'job:completed', 'job:error'])`. Prefer these helper-first signals (plus the inherited `job:*` lifecycle) over the legacy `RAP_TS_TASK_COMPLETED` DOM trigger when wiring dashboards or neighboring controllers.
- Transport: job submissions hit `WCHttp.postJsonWithSessionToken(url_for_run("acquire-rap-ts", { prefix: "/rq-engine/api" }), WCForms.serializeForm(form, { format: "json" }))`. The route parses inputs through `parse_request_payload`, normalizes optional `datasets`, `schedule`, and `force_refresh`, then enqueues `fetch_and_analyze_rap_ts_rq(payload=...)` so the worker can log metadata before running the climate-driven acquisition pipeline.
...
- Transport: queue submissions call `WCHttp.postJsonWithSessionToken(url_for_run("post-dss-export-rq", { prefix: "/rq-engine/api" }), { dss_export_mode, dss_export_channel_ids, dss_export_exclude_orders })` so session JWTs are minted on demand. The rq-engine route mirrors Flask’s `parse_request_payload`, normalizes JSON or legacy form posts to native ints/booleans, derives channel IDs from `Watershed.chns_summary` for mode 2, and persists via `wepp.dss_export_mode`, `wepp.dss_excluded_channel_orders`, and `wepp.dss_export_channel_ids`.
- Tests: front-end assertions in `controllers_js/__tests__/dss_export.test.js`; backend coverage in `tests/weppcloud/routes/test_rq_api_dss_export.py` using `tests/factories.singleton_factory` + `tests/factories.rq`. Execute `wctl run-npm lint`, `wctl run-npm test -- dss_export`, rebuild the bundle, and run the targeted pytest plus `wctl run-pytest tests --maxfail=1` (requires `node` for StatusStream) before handoff. Record new payloads and events in the current canonical domain contract.
...
- Transport: uploads stream `FormData` to `/rq-engine/api/batch/_/<name>/upload-geojson` (and `/rq-engine/api/batch/_/<name>/upload-sbs-map`); template validation and directive updates post JSON to `/batch/_/<name>/validate-template` and `/batch/_/<name>/run-directives`; run submissions post JSON to `/rq-engine/api/batch/_/<name>/run-batch`. The Flask routes now call `parse_request_payload`, and `BatchRunner.update_run_directives` converts `"true"/"false"` style strings to native booleans before persisting state.
- Tests: front-end coverage in `controllers_js/__tests__/batch_runner.test.js`; backend coverage in `tests/weppcloud/test_batch_runner_endpoints.py` (upload/validation/directives) and `tests/weppcloud/routes/test_rq_api_batch_runner.py` (queue wiring). Run `wctl run-npm test -- batch_runner`, rebuild the bundle, and execute the targeted pytest suites when touching this controller. Record payload schemas, event contracts, and open questions in the current canonical domain contract.

### AgFields controller reference
- DOM hooks: `controls/ag_fields_pure.htm` implements the observed five-stage layout described by `wepppy/nodb/mods/ag_fields/ui_control_layout.md` under `#ag_fields_form`, with the mapping dialog at `#agfields_rotation_modal`. That layout document is evidence, not a current canonical contract, until its registered child package ratifies it. Keep all interactions delegated through `data-action` and all rendered state on the documented `data-role` hooks. Stage 5 uses `data-role="integration-scheme-select"` plus scheme-suffixed status/result/limitation roles for `concept_1`, `concept_2`, and `hybrid`; `all` is selection-only and never receives a result row.
- Transport and telemetry: every route is under rq-engine `/api` and uses the existing session bearer-token helpers. The controller resolves the exact Stage 2-4 keys plus `agfields_run_watershed_concept_1`, `agfields_run_watershed_concept_2`, and `agfields_run_watershed_hybrid`, attaches the `ag_fields` StatusStream channel, and treats `GET .../agfields/state` as the only gating authority. The historical `agfields_run_watershed` key remains a Concept 2 alias. Run All tracks the complete returned `job_ids` mapping; a 409 `agfields_job_active` response must retain the active stream.
- Map: the sub-fields resource requires bearer authentication, so pass an authenticated `loadJson` callback to `MapController.addGeoJsonOverlay`. Auto-register current sub-fields during hydration, force the named `AgFields Sub-fields` overlay visible and refresh it after successful rebuilds, and leave ordinary visibility changes to the shared layer control without unregistering the overlay. Re-showing a remote GeoJSON overlay must rebuild a fresh Deck layer descriptor from cached data because a hidden descriptor may have been finalized.
- Tests: front-end coverage lives in `controllers_js/__tests__/ag_fields.test.js` with the shared loader assertion in `map_gl.test.js`. Run `wctl run-npm test -- ag_fields map_gl`, rebuild the bundle, and execute the targeted run-page/registry pytest suites before handoff.
