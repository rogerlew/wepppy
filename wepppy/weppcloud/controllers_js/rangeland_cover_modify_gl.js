...
var RangelandCoverModify = (function () {
    ...
    function ensureSelectionUtils() {
        var utils = typeof globalThis !== "undefined" ? globalThis.WCSelectionUtils : null;
        if (!utils) {
            throw new Error("RangelandCoverModify GL requires WCSelectionUtils.");
        }
        return utils;
    }
    ...
    function createInstance() {
        ...
        var selectionUtils = ensureSelectionUtils();
        var createLegacyAdapter = selectionUtils.createLegacyAdapter;
        var toResponsePayload = selectionUtils.toResponsePayload;
        var normalizeTopazId = selectionUtils.normalizeTopazId;
        var parseTopazField = selectionUtils.parseTopazField;
        var sortIds = selectionUtils.sortIds;
        var hexToRgba = selectionUtils.hexToRgba;
        var resolveMapElement = selectionUtils.resolveMapElement;
        var resolveMapCanvas = selectionUtils.resolveMapCanvas;
        var resolveEventLatLng = selectionUtils.resolveEventLatLng;
        var buildSelectionBounds = selectionUtils.buildSelectionBounds;
        var buildSelectionBoxLayer = selectionUtils.buildSelectionBoxLayer;
        var dom = helpers.dom;
        ...
        function updateSelectionRect(map, bounds) {
            ...
            var nextLayer = buildSelectionBoxLayer(deckApi, bounds, SELECTION_BOX_LAYER_ID);
            if (!nextLayer) {
                return;
            }
