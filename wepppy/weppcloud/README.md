...
| `REMEMBER_COOKIE_REFRESH_EACH_REQUEST` | `false` | Reserved; global Flask-Login refresh stays disabled because WEPPcloud refreshes only valid presented remember cookies |
...
- Remember-me cookies default to a rolling 90-day browser inactivity window with
  `Secure=True`, `HttpOnly=True`, and `SameSite=Lax`. Refresh is opt-in-aware:
  ordinary authenticated sessions do not acquire remember cookies. The duration
  can be overridden with `REMEMBER_COOKIE_DAYS`; global Flask-Login refresh is
  intentionally disabled.
- Authentication events append to `/wc1/logs/weppcloud/security.log` with
  directory mode `0700` and file mode `0600`. Install
  `docker/logrotate/weppcloud-security` as
  `/etc/logrotate.d/weppcloud-security` on production hosts; it retains 30
  daily compressed rotations, caps active-file growth at 20 MiB, and uses
  rename-and-create rotation with the application's watched file handler.
